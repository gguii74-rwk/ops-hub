...
| **D2** | **마이그레이션 = distinct `department` → `Team`.** 비-null `department` 고유값마다 `Team`(name=department) 생성, 해당 사용자 `teamId` 연결. `department`가 null인 사용자는 무소속. 이관 후 `department` 컬럼은 **같은 마이그레이션에서 drop**(F7 ACCEPTED — 즉시 drop). **이유**: 단일 pm2 인스턴스·`stop→migrate→start` 수동 배포·pre-cutover·16명 규모라 배포 중 version skew가 사실상 없고, expand/contract의 죽은 컬럼·후속 작업 비용이 이득을 넘어선다. **보완(필수)**: ① 마이그레이션 전 **DB 백업**(deploy 런북 선행 단계), ② **롤백 = 백업 복원** — 이 마이그레이션은 코드 롤백 불가 경계임을 런북에 명시, ③ `department` reader는 §7대로 **같은 릴리스에서 전부 전환**(구버전 코드가 drop된 컬럼을 못 읽는 잔존 경로 0). |
...
| **D8** | **scope 선택 제약(소비처 정합).** 매트릭스 편집기의 `team`/`own` 허용은 **본 증분에서 scope-aware로 이행하는 resource만** — `calendar.*`·`leave.*`(§7). `admin.*`·`integrations.*`·**`workflows.*`는 `all`만**(team/own 비활성). workflow scope 재설계는 OUT이라 소비처가 scope-aware가 아니므로, team/own을 열면 메뉴는 보이는데 unscoped workflow API가 403 나는 불일치가 생긴다(F5). 편집기가 옵션을 제한하고, 우회해도 fail-closed(D4). workflow scope는 후속 증분에서 소비처와 함께 연다. **〔plan 보정 PD2/F-F〕** 구현 단계에서 실제 scopeable 집합은 `leave.approval`만으로 좁혀졌다 — `calendar.*`·기타 `leave.*`(`leave.request`/`leave.status` 등)는 소비처가 all-scope `requirePermission`이라 열면 F5가 재발한다. **SSOT = `SCOPEABLE_RESOURCES`(plan task-02)**; 엔진 clamp로 그 외는 fail-closed. 상세: `docs/plans/2026-06-23-teams-and-permission-matrix.md` §PD2. |
...
**목록 필터 패턴(소비처)** — team scope는 **actor.teamId null 가드 fail-closed**(F9: `teamId=null` 필터는 무소속 버킷 전체를 매칭해 누수):
```ts
const scope = await getEffectiveScope(userId, "leave.approval", "view");
if (!scope) throw new ForbiddenError();
if (scope === "team" && myTeamId == null) throw new ForbiddenError();   // 무소속 team-scope actor → 거부(null 팀으로 필터하지 않음, F9)
const where = scope === "all" ? {} : scope === "team" ? { user: { teamId: myTeamId } } : { userId };
```
(단건 경로 `requirePermissionForTarget`은 team에서 `target.teamId != null` 요구라 actor null이면 `null !== 비-null` → 이미 거부; 위 null 가드는 목록 경로 전용.)
...
**`department` 컬럼 drop(D2)에 따른 필수 동반 수정(F8 — 전수 의무)** — D2가 컬럼을 제거하므로 `department`를 읽거나 쓰는 코드는 **하나도 빠짐없이 같은 릴리스에서** `team`(`teamId`/`team.name`)으로 전환해야 한다(누락 시 컴파일/런타임 실패). 2026-06-23 `\bdepartment\b` 스캔 기준 **20개 파일**에 걸쳐 있다 — 대표적으로:

- 사용자 관리: `modules/admin/users/{validations,services,repositories}`, `app/(app)/admin/users/**`(create/edit/approve/list)
- signup: `app/api/auth/signup/route.ts`, `modules/admin/users/validations/signup.ts`, `app/signup/_components/signup-form.tsx` — **`department`는 희망값(user-mgmt D9)**
- 연차: `modules/leave/services/{requests,status,users,calendar}`, `app/(app)/leave/_components/{status-client,user-select,admin-history}`, `app/(app)/leave/manage/approvals-client.tsx`, `app/api/leave/calendar/route.ts`, `app/api/admin/leave/status/export/route.ts`

**impl 수렴 기준(DEFERRED_TO_IMPL → §10·§11 AC)**: 목록을 손으로 고정하지 말고 **repo 전역 검색으로 전수 확정**하고, **검증 게이트**(grep/CI: 마이그레이션 파일 외 `department` 참조 == 0, 테스트 포함)를 통과해야 drop이 허용된다. signup의 `department` **희망값**은 curated Team(1인 1팀·관리자 배정)에 직접 매핑되지 않으므로 처리 방식(희망 텍스트 제거 vs 별도 비권위 컬럼 보관)을 impl에서 결정한다(§12).
...
- **무소속 team-scope 누수 방지(F9, 필수)**: teamId 없는 team-scope actor → 목록·단건 모두 **fail-closed**(무소속 버킷 전체가 노출되지 않음).
- **마이그레이션 정합성**: department→Team 집합 일치, null→null.
- **department drop 게이트(F8, DEFERRED_TO_IMPL · 필수 AC)**: drop 마이그레이션 적용 전 **마이그레이션 파일 외 `department` 참조 == 0**(grep/CI, src+tests 전역) AND `typecheck`/`build` 통과를 게이트로 강제 — 전수 전환을 기계 검증.
...
- ~~`department` 컬럼 즉시 drop vs deprecated 유지~~ → **결정됨: 즉시 drop + 백업/롤백 안전장치**(D2, 적대검증 F7 ACCEPTED)
...
- signup `department` 희망값(user-mgmt D9) 처리: Team이 curated(1인 1팀·관리자 배정)라 자유텍스트 희망부서가 직접 매핑 안 됨 → 희망 텍스트 제거 vs 별도 비권위 컬럼 보관(F8 동반, impl 확정)

## 부록: 적대검증 ledger (review-loop, spec phase)

codex 적대검증 5라운드(`--auto-rounds 3 --max 5 --base main`). 미판정 blocking 0으로 종료.

| # | finding | sev | disposition | 근거/연결 |
| --- | --- | --- | --- | --- |
| F1 | `getEffectiveScope` rank가 미해석 `assigned`를 `own`보다 위에 둬 유효 grant 가림 | high | FIXED | D3·§5: enforceable scope만 후보, assigned 제외 |
| F2 | `hasPermission` 계약을 any-scope로 바꾸면 서버 authz 상승·D4 자기모순 | high | FIXED | D5·§5·§7: any-scope는 `getPermissionSummary`만, hasPermission/requirePermission 무변경 |
| F3 | `leadUserId` 무제약+알림 포함 → 교차팀 누수 | high | FIXED | D1·§9·§10: lead ∈ active 팀원 불변식 |
| F4 | bootstrap-if-empty가 기존 설치의 신규 grant skip | medium | FIXED | D10: 멱등 마이그레이션이 신규 grant도 upsert |
| F5 | `workflows.*` team/own 허용 → 메뉴/API 불일치 | medium | FIXED | D8: workflows.*는 all-only |
| F6 | department→Team 마이그레이션 unqualified `User`+무검증 drop | high | FIXED | §4: 스키마 정규화·단일 트랜잭션·drop 전 사전 단언 |
| F7 | 즉시 destructive drop의 롤백/version skew 위험 | high | ACCEPTED | D2: 백업+롤백=복원+단일 인스턴스 skew 무시(사용자 결정) |
| F8 | department reader를 3개로 축소 명시(실제 20파일) → drop 시 광범위 breakage | **critical** | FIXED + **DEFERRED_TO_IMPL** | §7·§10: 전수 전환 의무 + **impl AC**(마이그레이션 외 `department` 참조 0 grep/CI 게이트). signup 희망값 처리는 §12 |
| F9 | team-scope 목록 필터가 actor.teamId null 미가드 → 무소속 버킷 누수 | high | FIXED | §5·§10: 무소속 team-scope fail-closed |

**plan으로 이월(필수 AC)**: F8의 "마이그레이션 외 `department` 참조 0" 검증 게이트(grep/CI) + 전수 전환을 impl plan의 acceptance criteria/task로 연결한다. signup 희망값 처리(§12)도 plan에서 확정.
