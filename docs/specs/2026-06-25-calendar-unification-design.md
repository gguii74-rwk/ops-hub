...
> 적대검증(adversarial review)은 아래 결정·상위 캘린더/연차 spec·메모리 디자인 방향을 모른다. finding이 이 결정과 충돌하면 버그가 아니라 의도된 설계다 — 고치기 전 대조할 것. 특히 D10(불변식 보존)을 "표현 계층이 도메인 로직을 호출하지 않는다"는 누락으로 오인하지 말 것.
...
| **D8** | **셀 클릭 → 팝오버. 내용·액션은 호출부 주입.** `CalendarMonth`가 팝오버 컨테이너(위치 계산·뷰포트 클램프·Esc·포커스 트랩·바깥 클릭 닫기)를 제공하고, **내용은 `renderDayDetail(ctx)` prop**으로 주입받는다. 통합 = **읽기전용**(그 날 이벤트 상세). 연차 = 그 날 연차 목록 + **능력별 신청 액션**: `canCreate`(=`leave.request:create`)면 자가신청→`/leave/request?date=`(라우트 보존), `canManage`(=`approvalScope==='all'`)면 관리자 직접입력→`CreateLeaveModal`. **두 경로는 별개**(능력·제출 경로가 다름) — 하나로 합치면 일반 사용자 자가신청 회귀(plan review R1). 도메인 규칙(과거 날짜 신청 차단 등)은 **주입 액션 쪽/대상 라우트**에서 처리 — 컴포넌트는 모른다. |
| **D9** | **hover 빠른추가(+) = `onQuickAdd?(dateKey)` 주입 시에만 노출(마우스 hover affordance).** 미주입이면 미표시. 연차는 **자가신청 능력(`canCreate`)** 사용자에게 노출(→`/leave/request?date=`), 통합은 미주입=읽기전용. **키보드/모바일 사용자**는 셀 포커스→Enter(또는 탭)로 팝오버를 열어 그 안의 신청 액션(D8)을 쓴다 — 빠른추가(+)는 보조 단축일 뿐 유일 경로가 아니다(키보드·터치 접근성 보장). |
...
| **D11** | **연차 진입점 흡수(라우트·버튼 → 팝오버/빠른추가) — 두 경로 각각 보존.** 기존 ① 날짜 클릭→`/leave/request?date=`(자가신청, 모든 뷰어)와 ② 상단 `+ 연차 입력`(관리자 직접입력, `canManage`)을 **각각** 팝오버 액션·빠른추가(+)로 옮긴다. ① 자가신청은 `canCreate`로 게이트하고 **`router.push('/leave/request?date=')` 라우트를 그대로 사용**(페이지가 `leave.request:create` enforce). ② 관리자 직접입력은 `canManage`로 게이트하고 기존 `CreateLeaveModal`(→`/api/admin/leave/requests`) 재사용. **둘을 `canManage` 하나로 병합 금지**(일반 사용자 자가신청 회귀 — plan review R1). **신청 폼·검증·제출 경로 무변경**. 라우트 `/leave/request`는 사이드바 트리 항목으로도 존속(사이드바 spec D3). |
