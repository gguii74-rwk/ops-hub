...
export interface CalendarEventInput {
  ...
  end?: string;          // ISO instant. 없으면 단일일(start 당일).
...
- **어댑터(소비처)**:
  - 통합 `calendar-view.tsx`: `CalEvent` → `{ id, title, kind: e.kind, start, end, status: undefined }`(통합은 status 개념 없음; `tentative`는 PENDING 오버레이로 매핑 가능).
  - 연차 `leave-calendar.tsx`: `Ev` → `{ id, title: name + getFullLeaveText(...), kind: leaveType, start: startDate, end: endDate, status }`.
...
- `packWeekLanes`: 단일일/기간/겹침 2건→2 lane/주 경계 분할(◂▸)/overflow(+N)/정렬 안정성.
...
2. 기간 이벤트가 **연속 막대**로 보이고(주 경계 ◂/▸), 겹치면 lane으로 분리된다.
