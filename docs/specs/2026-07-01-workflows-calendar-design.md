...
- `useQuery(["workflows","calendar",startKey,endKey], () => fetch(\`<calendar range endpoint>?start=${encodeURIComponent(startKey)}&end=${encodeURIComponent(endKey)}\`))` — **start/end 값을 반드시 실어 그리드 42칸 윈도우만 조회**. 클라가 값을 실을 뿐 아니라 **서버가 range 필수·span cap을 강제**(D5·R3·F1) — 빈/무범위로 전체 이력을 반환하지 않는다. 상태 필터는 없음(취소 포함 — §4.4). (엔드포인트 확정은 D5의 DEFERRED_TO_IMPL.)
