...
**seed create-only 백필(D6, F5·F6·F8)** — `prisma/seed.ts`: HOLIDAY 소스 upsert는 유지. `calendarIds`/`calendarOwners` 기반 GOOGLE_CALENDAR 처리를 **create-only**로 축소한다 — calId가 기존 CalendarSource(externalId 매칭)에 **없으면 생성(ACTIVE)**, **있으면(어떤 syncStatus든) 아무 것도 하지 않는다**(`planGoogleSources`의 update·deactivate/PAUSED 경로 삭제, create만 유지). 재-seed가 기존(수동/CRUD/PAUSED) 소스를 절대 바꾸지 않으면서 신규/미변환 env 부트스트랩은 유지된다. **재활성화는 비목표**(F8): listed인데 PAUSED인 행은 백필이 켜지 않는다(의도적 pause 보호). 컷오버 시점엔 listed가 모두 ACTIVE라 잔존 비활성 없음; PR-B 전 PAUSED 복구는 수동 `UPDATE`로 문서화. PR-B 도입 후 calId 완전 폐기는 follow-up.
...
- **integrations 상태**: `smtpConfigured`(F9 auth 분기) — ① host+`SMTP_USER`+`SMTP_PASSWORD` 있음→정상, ② host 있고 `SMTP_USER` **없음**(무인증 릴레이)→**정상**, ③ host 있고 `SMTP_USER` 있는데 `SMTP_PASSWORD` 없음→설정 필요, ④ host 없음→설정 필요, ⑤ 무효 DB port여도 host 조건 충족 시 정상. `googleConfigured`(소스 카운트·secret 조합), Google·문서 `unknown` 환원 회귀.
- **catalog/service**: `integrations.smtp.host` 제거·`secure`/`user` 미추가, group/groupOrder 전파, `calendarIds` relational 전환.
- **seed create-only 백필(F5·F6·F8)**: ① `db:seed` 재실행이 기존 GOOGLE_CALENDAR 소스(수동/CRUD 추가·**PAUSED 포함**)의 `syncStatus`/필드를 **바꾸지 않음**(드리프트 회귀 + PAUSED 비재활성화), ② calIds에 있고 CalendarSource에 **없는** calId는 ACTIVE로 **생성됨**(부트스트랩), ③ HOLIDAY 소스 upsert 유지.
...
| F8 | create-only 백필이 listed-but-PAUSED 소스를 재활성화 못 함(복구 보장 과장) | high | **FIXED** | D6·§5.1 — 복구 계약을 "없는 소스 생성만"으로 정밀화. 컷오버 시 listed는 모두 ACTIVE(구 seed)라 잔존 비활성 없음; 의도적 PAUSE는 보호(맹목 재활성화 금지), PR-B 전 복구는 수동 `UPDATE`로 문서화. §7 PAUSED 비재활성화 회귀. |
...
**적대검증 라운드 추세(미판정 blocking score, weight critical=4·high=3·medium=1)**: R1=6(F1·F2) → R2=1(F3) → R3=6(F4·F5) → R4=6(F6·F7) → R5=4(F8·F9). 각 라운드가 새 실재 엣지를 드러냈고(severity 별 fingerprint 비중복), 모두 FIXED/ACCEPTED로 닫음. R5에서 max(5) 도달 — 잔여 미판정 blocking 0(F1~F9 전부 판정 완료). FIXED 항목은 impl 단계에서 §7 회귀 테스트로 재검증한다.
