...
- 날짜: 2026-06-26
- 상태: 설계 승인됨 (2026-06-26) — PR-A plan 작성 완료. **plan-review에서 D11 반영**(port를 env 전용으로 — P3/A2; DB 편집 SMTP 필드 = fromAddress 하나).
...
| PR | 범위 |
| --- | --- |
| **PR-A** | 설정 페이지 IA 재편 + SMTP 상태 진실화·**fromAddress DB 배선**(host·user·secure·port는 env, D2·D11) + 편집기(string/number/list 타입 dispatch) + 나열형(calendarIds·defaultRecipients) 리스트 편집기. **Google 소스 관리는 손대지 않음**(calendarIds는 기존 systemSetting 그대로 유지). |
...
- **D1 — 연동 설정 진실원 = UI(DB), 단 SMTP 연결/인증 민감 필드는 env(보안).** 비민감 SMTP 필드(**fromAddress**)는 DB가 진실원이고 전송이 이를 읽는다(env fallback). 그러나 **연결/인증 민감 필드(host·user·secure·password)는 env에 유지**한다 — DB-편집 host에 전역 env password를 주입하면 비밀번호 유출 벡터가 생기기 때문(F4, §10). **`port`도 env 전용**(D11/A2 — TLS 모드 `secure`와 결합돼 함께 env 관리). 상태 점검은 실제 동작 출처(env host+password+port, DB-or-env from)를 본다.
- **D2 — SMTP 연결/인증 정보(host·user·secure·password) + port는 env 전용.** 이들은 비암호 SystemSetting/감사 로그에 두지 않고 env 신뢰경계에 유지한다(F4 유출 벡터 차단; port는 D11/A2 — port/TLS 드리프트 차단). **DB로 편집 가능한 SMTP 필드 = fromAddress 하나뿐**(비민감). password는 상태 배지(env secret)만. host/user/secure/**port**는 UI에 편집 노출하지 않는다(서버 env로 관리; 읽기전용 표시는 후속 선택).
...
- **D8 — 편집기는 값 타입으로 선택.** `string`→텍스트 입력, `number`→숫자 입력, `boolean`→스위치(기존), `string[]`→리스트 편집기, 그 외(객체)→기존 JSON textarea 폴백. 서버 zod가 여전히 권위 검증. (D11/A2로 port가 env 전용이 되어 현재 numeric systemSetting은 없음 — number 편집기는 타입-완전 dispatch로 유지하되 production 미사용.)
...
- **D11 — `port`는 env 전용(plan review 갱신, P3/A2).** 최초 D1/D2는 port를 DB-편집(비민감)으로 뒀으나, plan 단계 적대검증(R3)에서 **port와 TLS 모드(`secure`)의 결합**이 드러났다 — DB port와 env `SMTP_SECURE`가 어긋나면 상태 배지는 "정상"이나 실제 미연결(정적 탐지 가능한 로컬 불변식). 사용자 결정(A2): **port도 env 전용**으로 옮겨 port/secure를 env 한 곳에서 관리(드리프트 원천 제거). 결과: **DB 편집 가능한 SMTP 필드 = fromAddress 하나뿐.** `integrations.smtp.port` systemSetting은 카탈로그에서 제거(host와 동일), `getSmtpConfig`는 port를 `process.env.SMTP_PORT`(없으면 587)만 읽고 DB row는 orphan으로 무시. 이 결정이 아래 §5 본문·§7 테스트·§10 F7의 "DB port" 표현을 **개정**한다(본문은 D11 기준으로 읽는다).
...
| mail | 메일 (SMTP) | `secret.smtp`(상태/env), `integrations.smtp.fromAddress`(DB) |
...
**SMTP 카탈로그 변경(D2, F4, D11)**:
- `integrations.smtp.host`·`integrations.smtp.port` systemSetting **제거**(env 전용 — host=F4, port=D11/A2; UI 편집 노출 안 함). orphaned DB row는 무해(getEntry undefined → 미표시·미기록·비쓰기·미읽기). 현재 이를 읽는 곳은 status.ts/transport뿐이며 모두 env 기반으로 재작성(§5.2·§5.3)하므로 안전.
- `integrations.smtp.fromAddress` systemSetting **유지**(DB 편집, 전송이 읽음 — §5.2). **DB 편집 가능한 SMTP 필드는 fromAddress 하나뿐.**
- `secure`/`user`/`port`는 **DB에 추가/유지하지 않는다**(env 전용). (이전 안의 TLS 토글·사용자명·포트 편집 필드 폐기 — port는 D11로 env 회귀.)
...
//   port:   Number(env SMTP_PORT) || 587    ← env 전용(D11/A2 — DB row 미읽음)
//   from:   DB fromAddress(유효 이메일) || env SMTP_FROM || env SMTP_USER || "noreply@uracle.co.kr"  ← DB 편집(env fallback)
```
- **민감 필드(host/user/secure) + `port`는 env에서만 읽는다**(D2·D11). DB는 읽지 않으므로 유출 벡터·throw·port/TLS 드리프트 없음.
- **DB-편집 필드는 `from` 하나뿐. `readRaw`(원시 행)로 읽고 `getSetting` 금지(F7 정신).** `getSetting`은 행 부재 시 **카탈로그 default를 반환**해 빈값/부재를 구분 못 한다. 따라서 **행 부재/빈값(`readRaw`=null 또는 `""`)** → env(`SMTP_FROM`→`SMTP_USER`→default), **행 존재+유효(카탈로그 email schema 통과)** → DB, **행 존재+무효(비-이메일)** → env(P1). (port는 env 전용이라 readRaw 대상 아님 — D11.)
- **폴백 규칙(D1)**: `from`은 DB 행이 유효하면 DB, 없거나 비었거나 무효면 env. host/user/secure/port는 항상 env.
- **tolerant(D10)**: DB-읽기 필드(`from`)만 try/catch로 감싸 무효 파싱·읽기 실패 시 env로 폴백하고 `console.warn`만 남긴다. **`getSmtpConfig` 전체가 throw하지 않는다.**
...
- `smtpConfigured()`(F9, 전송 auth 분기와 일치): `cfg.host.length > 0` AND **인증 정합성** — `SMTP_USER` 미설정이면 무인증 릴레이로 발송되므로 `host`만으로 OK, `SMTP_USER` 설정 시에만 `SMTP_PASSWORD`(env) 존재를 요구. 즉 `host && (!SMTP_USER || (SMTP_USER && SMTP_PASSWORD))`. host는 env(D2)에서 오므로 "env에 발송 가능한 SMTP가 있으면 정상" = 실제 발송 가능 여부와 일치(원래 모순·F9 false-negative 모두 해소). SMTP는 throw하지 않으므로 `safe()` 래핑 불필요 — `unknown` 미발생.
  - **`configured` 계약(명시)**: 정적으로 검증 가능한 **필수 설정의 존재·정합성**만 본다 — host(env) + 위 인증 정합성. `from`은 tolerant 해석기가 항상 유효값 보장하므로 추가 게이트 불필요. host·user·secure·password·port가 모두 env 신뢰경계에 있어(D2·D11) DB-편집發 불일치 표면은 `fromAddress`뿐(F3 우려가 F4·D11로 완화). 다만 배지는 "**메일이 반드시 성공한다**"는 보장이 **아니다** — 서버 도달성·인증 성공은 라이브 핸드셰이크 없이는 알 수 없다. 실제 발송 확정은 후속 "연결 테스트" 액션(§10)으로 분리. 잘못 저장된 `from` 행은 항목별 INVALID 배지로 노출.
...
- **kernel**: `getSmtpConfig` — host/user/secure/**port**는 **env에서만** 해석(DB row가 있어도 무시, D11), `from`만 DB 우선·env 폴백·빈 값 처리.
...
- **kernel(getSmtpConfig tolerant, D10)**: ① **무효 DB from row + 유효 env SMTP → throw 없이 env로 해석, 발송 가능**(무회귀·P1), ② settings 읽기 실패(인프라 오류) + env present → env config 반환·throw 없음, ③ DB from 채움(유효 이메일) 시 DB 우선·빈 값/무효 시 env 폴백 검증.
- **kernel(port env 전용, D11)**: **`SMTP_PORT=465` → 465**, env 없음→587, **DB `integrations.smtp.port` row가 있어도 무시**(env 값 사용), `SMTP_PORT` 무효(NaN)→587.
- **integrations 상태**: `smtpConfigured`(F9 auth 분기) — ① host+`SMTP_USER`+`SMTP_PASSWORD` 있음→정상, ② host 있고 `SMTP_USER` **없음**(무인증 릴레이)→**정상**, ③ host 있고 `SMTP_USER` 있는데 `SMTP_PASSWORD` 없음→설정 필요, ④ host 없음→설정 필요. (port는 env라 상태 판정에 미관여.) `googleConfigured`(현행: secret+calendarIds), 문서 `unknown` 환원 회귀.
- **secret.smtp 행 상태(F12)**: `SMTP_USER` 없음→행이 "설정 필요"로 안 뜸(중립), `SMTP_USER` 있고 password 없음→설정 필요. 그룹 헤더와 항목 행 일관성 검증.
- **catalog/service**: `integrations.smtp.host`·`integrations.smtp.port` 제거·`secure`/`user` 미추가(DB 편집 SMTP=fromAddress뿐), group/groupOrder 전파, **`calendarIds`는 systemSetting 유지(relational 전환 안 함)**.
- **UI(편집기)**: list 추가/삭제/형식검증(calendarIds·defaultRecipients), string 편집기 저장·롤백(rejected)·refetch 경로, number 편집기 분기(합성 키 — production numeric 설정 없음, D8/D11), boolean 회귀.
- 기존 스위트 그린 유지(현재 1380 통과 기준).

## 8. 배포 / 마이그레이션

- **Prisma 마이그레이션 없음 + seed 무변경**(D9, PR-A) → 표준 restart 배포(`build` → `pm2 restart`). PR-A는 seed·권한 catalog를 바꾸지 않으므로 `db:seed` 필수 아님(신규 SystemSetting 키 미추가, host·port 제거는 카탈로그에서만). (seed cutover는 PR-B에서 `db:seed` 실행 동반.)
- 서버 env(`SMTP_HOST`/`SMTP_USER`/`SMTP_SECURE`/`SMTP_PORT`/`SMTP_PASSWORD` 등)는 그대로 유지 — 민감 연결/인증 필드 + port의 진실원이자(D2·D11), `from` DB 미입력 시 폴백원. DB 미입력 상태에서도 현행 메일 발송 무회귀.
- **cutover preflight(P2/B1, 필수)**: 死설정이던 `integrations.smtp.fromAddress` 행이 살아나므로, restart 전 `SELECT value FROM "SystemSetting" WHERE key='integrations.smtp.fromAddress'`로 비어있지 않은 stale 값이 env `SMTP_FROM`과 어긋나는지 확인·정리한다(plan §DoD). 마이그레이션 아님(D9). port는 env 전용(D11)이라 cutover 대상 아님.
- smoke: 인증 후 `/admin/settings` 렌더(영역 카드·상태 배지), SMTP **fromAddress** 저장→메일 발송 1건(발신주소 반영), advisory 라우트 회귀(stale-build P2010 주의 — 인증 경로 포함).

## 9. 영향 범위 / 리스크

- **메일 전송 경로 변경**(working path) — D1 폴백 + D10 tolerant 해석기로 회귀 위험 차단(깨진 DB `from` 행이 env 발송을 막지 못함). 가장 큰 검증 포인트: config 미주입(테스트)·DB from 빈 값(폴백)·DB from 채움(신규)·**DB from 무효 행+env 유효(throw 없이 발송, P1)** 4경우. (port는 env 전용이라 DB-발 회귀 없음, D11.)
- `sendMail` 시그니처 확장 — 호출자 2곳(leave·workflows) 동시 갱신 필수.
- 표현계층(IA·편집기) 변경은 도메인 불변식·동시성 패턴 무영향(설정 쓰기 토큰 패턴·audit 유지).
- **보안**: SMTP 연결/인증 민감 필드(host·user·secure·password) + port를 env에 유지(D2·D11, F4) — DB-편집 가능한 SMTP 표면은 비민감 `fromAddress` 하나로 한정. 비밀번호 유출 벡터·port/TLS 드리프트 없음.
...
| F3 | SMTP 상태가 port/from/auth 일치 등 전송 전제를 무시 → "정상"인데 발송 실패 가능 | medium | **ACCEPTED** | `from`은 tolerant 해석기가 유효값 보장. host·user·secure·password·**port가 모두 env**(D2·D11)라 DB-편집發 불일치 표면은 `fromAddress` 하나(P1 검증). 서버 도달성·인증 성공은 **라이브 핸드셰이크 없이 정적 검증 불가** — 배지 계약을 "필수 설정 존재"로 명시(§5.3). **보완 단계(후속)**: SMTP "연결 테스트" 액션. (plan-review P3: port/TLS 결합 위험은 port를 env로 옮겨 제거 — D11.) |
| F4 | DB-편집 host에 전역 env password 주입 → 임의 host로 비밀번호 유출 | high | **FIXED(설계 변경)** | 사용자 결정(option 1): host·user·secure·password를 **env 전용**으로 유지(D1·D2 개정). DB 편집은 비민감 `fromAddress`뿐(port도 plan-review D11/A2로 env 전용). host가 UI 통제 밖이라 유출 벡터 제거. §5.1 catalog에서 `integrations.smtp.host`·`port` 제거, `secure`/`user` 미추가. |
...
| F7 | `getSmtpConfig` port가 `getSetting`(행 부재 시 default 587)을 쓰면 비-587 env(465 등) 무시·발송 실패 | high | **FIXED** | 당초 §5.2에서 `readRaw`(행 부재/유효/무효 구분)로 해결했으나, **plan-review D11/A2로 port를 env 전용으로 옮겨 근본 차단**(DB port row 자체가 없음 → 마스킹 불가). `from`만 `readRaw` 처리. §7 `SMTP_PORT=465`→465·DB row 무시 회귀 테스트. |
