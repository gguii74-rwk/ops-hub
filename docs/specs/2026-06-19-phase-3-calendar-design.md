...
- **leave 뷰는 외부 '휴가'만 — 비휴가 Google 일정 제외(후속 적대적 리뷰).** leave 뷰는 google 소스를 owner 스코프하지 않고 전체를 가져온다(팀 부재 파악). 그런데 dedup은 `DUPLICATE_OF_INTERNAL`만 접고 일반 `EXTERNAL_EVENT`(휴가 아닌 바쁨 블록)은 남는다 — 마스킹은 제목만 가리고 start/end·userId는 남으므로 `calendar.leave:view`(외주 포함) 사용자가 타인 Google 캘린더의 비휴가 사적 시각을 추론할 수 있다. 따라서 feed가 **leave 뷰에서 `EXTERNAL_EVENT`를 제외**한다(EXTERNAL_VACATION·내부 데이터는 유지). admin 뷰는 진단 위해 전체 노출, personal은 본인 것만(아래).
...
- **외부 fetch 타임아웃 + 소스별 병렬(후속 적대적 리뷰)**: `allSettled`는 provider *reject*만 막고 *hang*은 못 막는다 — 멈춘 Google 호출(응답·에러 모두 없음)은 provider 프로미스를 영원히 미해결로 두어 feed 전체(이미 준비된 내부 휴가 포함)를 막는다. 따라서 external provider는 ① 소스별 `fetchWithTimeout`(상한 `EXTERNAL_FETCH_TIMEOUT_MS`)으로 감싸 초과 시 reject→`getCachedPayload`가 stale/failed로 환원하고, ② 소스 루프를 `Promise.allSettled`로 **병렬화**해 한 캘린더가 다른 소스를 직렬로 막지 않게 한다(N×timeout 방지). gaxios 요청별 `timeout`은 HTTP 계층 백스톱(멈춘 소켓 조기 차단). 회귀 테스트: never-resolving 클라이언트 → provider가 failed로 resolve(행 없음).
