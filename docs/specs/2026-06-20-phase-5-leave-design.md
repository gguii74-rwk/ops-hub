...
| D7 | 교차연도 ANNUAL 신청은 **허용하되 시작연도에 전체 일수 일괄 차감**(정밀 분할 안 함) | 마이너스 연차 허용을 활용해 연도 경계 총량은 **차년도 할당 생성 시 전년 마이너스 잔여 이월**로 수렴(§6, §12). 교차연도는 드물어 연도별 분할 회계는 과함(YAGNI) |
| D8 | 공휴일 미적재 시 **직원 신청 fail-closed**(차단), **관리자 경로는 경고만 통과** | 빈/오래된 공휴일로 계산한 `days`가 영속화되면 `recalculate`(저장 `days` 합산)로도 복구 불가 → 직원 신청은 차단. 관리자는 의도적 보정 주체라 경고만(#1). `count>0`(단일 트랜잭션 sync)=완전 적재 마커. 부팅+요청 backstop이 현재+익년을 채우고, 미적재는 admin 배너+수동 sync로 보강 |
...
- `@db.Date`로 시간대 잡음 제거(UTC 자정 기준 1일 단위). `getHolidaysInRange`는 `yyyy-MM-dd` 문자열 Set으로 환원해 `rules.ts`에 넘긴다.
- 채움: **공공데이터포털 한국천문연구원 특일정보 API**(`getRestDeInfo`, 대체공휴일 포함)에서 `Holiday` 테이블로 **동기화**한다. day-calc는 라이브 API가 아니라 **테이블을 출처로 읽으므로 결정적**이다(sync는 별도 경로). 동기화 트리거: ① 서버 부팅 시(`instrumentation.ts`)·시드 시 `ensureYearsSynced([현재연도, 익년])` — 서버는 배포·재시작마다 부팅하므로 **연도가 바뀌면 내후년이 자동 적재**된다, ② admin 수동 sync(특정 연도 강제 재동기화), ③ 요청 경로 backstop(아주 먼 미래 연차 신청 시 해당 연도 미적재면 ensure), ④ **admin 보강**(미적재 시 관리자 화면 배너 알림 + 수동 sync 버튼 — 성공할 때까지 접속 시 알림). 환경변수 `DATA_GO_KR_SERVICE_KEY` 필요.
- **장애 처리(fail-closed, D8)**: sync 실패(API 다운·키 오류)는 부팅 경로에서 **로그만 남기고 진행**한다(부팅을 막지 않음). 그러나 **직원 신청(`createLeaveRequest`)은 필요 연도가 미적재(`count===0`)면 폴백하지 않고 차단**한다(`LeaveValidationError`) — 빈/오래된 공휴일로 잘못된 `days`를 영속화하지 않기 위함(영속된 `days`는 `recalculate`로도 복구 불가). **관리자 경로(`createLeaveRequestByAdmin`·`updateByAdmin`)는 차단하지 않고 경고만 남기고 통과**한다(의도적 보정 주체 — #1). 부팅·시드·요청 backstop이 현재+익년을 채우고 미적재는 admin 화면 배너+수동 sync로 보강하므로, 직원 차단은 먼 미래 연도 등 예외에 한정된다. stale(임시공휴일 사후 지정 등)은 admin 수동 재sync로 복구(§12 후속: 주기적 재동기화).
...
- **유형·일수** `calculateLeaveDays(type, start, end, holidays: Set<string>)`:
  - `ANNUAL` = start~end 사이 **주말(토·일) 및 공휴일 제외 일수**. (POC는 주말만 제외 → 본 포팅에서 공휴일 제외 추가, D3.) **교차연도 기간도 전체를 한 값으로 계산**하며, 회계는 시작연도 할당에 일괄 차감한다(D7, §6).
  - `HALF` = 0.5, `QUARTER` = 0.25 (단일일 — 교차연도 불가).
- **날짜 검증**:
...
- `recalculate(userId, year)` (신규, admin): 해당 연도 `APPROVED` 합계로 `usedDays`를 재계산·확정. 장애·수동 DB 수정 후 캐시 정합성 복구. **저장된 `days`를 합산할 뿐 공휴일 기준으로 `days`를 재계산하지 않는다** — 그래서 잘못된 `days`의 영속화를 신청 시점에 막는다(D8).
- 할당 없는 사용자 승인/직접입력은 거부("할당 정보 없음") — POC 동작.
- **교차연도 신청(D7)**: `usedDays` 증감은 모두 `startDate`의 연도 할당에 **전체 `days`를 일괄** 적용한다(연도별 분할 없음). 시작연도 잔여가 마이너스가 될 수 있으며(마이너스 허용), 연도 경계 총량 정합은 **차년도 할당 생성 시 전년 마이너스 잔여 이월**로 맞춘다(이월 정산은 후속, §12).
- **공휴일 fail-closed(D8)**: **직원 `createLeaveRequest`** 는 일수 계산 전에 신청 기간이 걸친 연도의 공휴일 적재를 `getUnsyncedYears`로 확인하고, 미적재면 `LeaveValidationError`로 거부한다(폴백 안 함). **관리자 `createLeaveRequestByAdmin`·`updateByAdmin`은 미적재여도 경고(`console.warn`)만 하고 통과**한다(#1). 적재 확인 함수는 `kernel/holidays`가 노출(§7).
...
- `getHolidaysInRange(start, end): Promise<Set<string>>` — `Holiday`에서 범위 내 행을 읽어 `yyyy-MM-dd`(UTC) 문자열 Set 반환. 빈 결과여도 정상.
- `syncHolidaysForYear(year)` — 공공데이터 특일정보 API(`lib/integrations/holidays`)에서 해당 연도 공휴일을 가져와 `Holiday`에 **단일 트랜잭션**으로 upsert(부분 적재 없음 → `count>0`=완전). `ensureYearsSynced(years)` — 각 연도가 미적재(`count===0`)면 sync 시도(실패는 로그 후 진행 — 부팅을 막지 않음). `getUnsyncedYears(years): Promise<number[]>` — 인자 연도 중 **여전히 미적재(`count===0`)인 연도를 반환**(직원 fail-closed 게이트·admin 알림용).
- leave service가 신청·수정 시 spanned 연도를 `ensureYearsSynced`로 보장(sync 시도)한 뒤 `getUnsyncedYears`로 확인한다 — **직원 `createLeaveRequest`는 미적재면 `LeaveValidationError`로 차단**(fail-closed, D8), **관리자 경로는 경고만 통과**(#1). 통과하면 `getHolidaysInRange`로 Set을 조회해 `calculateLeaveDays`에 주입한다. 테이블이 출처라 **결정적**이다(라이브 캐시 아님).
- 자동 채움: `instrumentation.ts`(부팅)·시드가 `ensureYearsSynced([현재, 익년])`. 서버 재시작마다 도므로 연도 경과 시 내후년 자동 적재. 별도 백그라운드 타이머는 없고, 미적재는 **admin 화면 배너 알림 + 수동 sync**(`GET`/`POST /api/admin/leave/holidays/sync`)로 보강한다(성공할 때까지 접속 시 알림).
...
- `/admin/leave/allocations`: 연도 선택 → 사용자별 할당 표(설정·조정), 조정 이력, `recalculate` 액션, **공휴일 수동 sync 버튼 + 미적재 연도 경고 배너**(`getUnsyncedYears` 기반 — 미적재 시 직원 신청 차단 안내).
...
- 신청: 중복(PENDING/APPROVED 겹침) 거부 / 마이너스 허용(잔여 부족해도 생성) / HALF subType·QUARTER startTime 보존 / **교차연도 ANNUAL은 시작연도에 전체 일수 일괄 차감**(D7) / **직원 신청은 필요 연도 공휴일 미적재면 차단**(fail-closed, D8) / 관리자 경로는 미적재여도 통과(경고).
...
- `kernel/holidays`: 범위 조회가 yyyy-MM-dd Set, 빈 결과 정상 / `getUnsyncedYears`가 미적재(`count===0`) 연도 반환 / sync는 단일 트랜잭션(부분 적재 없음).
...
- **공휴일 동기화**: 공공데이터 특일정보 API → `Holiday` sync(부팅·시드·요청 backstop)로 현재+익년 자동 적재, 미적재 시 직원 신청 차단(fail-closed, D8) + admin 배너 알림·수동 sync. 후속: 기존 적재 연도의 **주기적 재동기화로 stale(대체공휴일 사후 추가) 반영**, `calendar`의 `Holiday` 리더 전환, 사내 임시휴무 등 비공식 휴일 수기 관리.
- **연도 경계 이월 정산**: 차년도 할당 생성 시 전년 마이너스 잔여(초과 사용분 — 교차연도 신청 포함)를 차년도에 이월·합산해 총량을 맞춘다(D7). Phase 5는 정책만 명시하고, 자동 이월 정산 기능(이월 만료·차년도 생성과 연계)은 후속.
