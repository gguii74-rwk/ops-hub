...
- 채움: **공공데이터포털 한국천문연구원 특일정보 API**(`getRestDeInfo`, 대체공휴일 포함)에서 `Holiday` 테이블로 **동기화**한다. day-calc는 라이브 API가 아니라 **테이블을 출처로 읽으므로 결정적**이다(sync는 별도 경로). 동기화 트리거: ① 서버 부팅 시(`instrumentation.ts`)·시드 시 `ensureYearsSynced([현재연도, 익년])` — 서버는 배포·재시작마다 부팅하므로 **연도가 바뀌면 내후년이 자동 적재**된다, ② admin 수동 sync(특정 연도 강제 재동기화), ③ 요청 경로 backstop(아주 먼 미래 연차 신청 시 해당 연도 미적재면 ensure). 환경변수 `DATA_GO_KR_SERVICE_KEY` 필요.
- **장애 격리**: sync 실패(API 다운·키 오류)는 로그만 남기고 진행한다. 이미 적재된 연도는 영향 없고, 미적재 연도는 그 구간 주말만 제외(공휴일 미반영)로 폴백 — admin이 sync로 복구. 부팅·시드에서 현재+익년을 미리 채우므로 라이브 의존은 예외적이다.
...
- `syncHolidaysForYear(year)` — 공공데이터 특일정보 API(`lib/integrations/holidays`)에서 해당 연도 공휴일을 가져와 `Holiday`에 upsert. `ensureYearsSynced(years)` — 각 연도가 미적재(`count===0`)면 sync(실패는 로그 후 진행 — 장애 격리).
- leave service가 신청·수정 시 spanned 연도를 `ensureYearsSynced`로 보장한 뒤 `getHolidaysInRange`로 Set을 조회해 `calculateLeaveDays`에 주입한다. 테이블이 출처라 **결정적**이다(라이브 캐시 아님).
- 자동 채움: `instrumentation.ts`(부팅)·시드가 `ensureYearsSynced([현재, 익년])`. 서버 재시작마다 도므로 연도 경과 시 내후년 자동 적재.
...
- **공휴일 동기화**: 공공데이터 특일정보 API → `Holiday` sync(부팅·시드·admin·요청 backstop)로 현재+익년 자동 적재. 후속: 기존 적재 연도의 주기적 재동기화(대체공휴일 사후 추가 반영), `calendar`의 `Holiday` 리더 전환, 사내 임시휴무 등 비공식 휴일 수기 관리.
