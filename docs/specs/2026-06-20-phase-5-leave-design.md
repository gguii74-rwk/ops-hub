...
- 시드: `prisma/seed-holidays.ts`(또는 seed에 포함)로 한국 공휴일 정적 데이터를 적재한다. **적재 범위 = 시드 실행 시점의 현재 연도 + 익년**(예: 2026년 실행 시 2026–2027). 결정적·외부 의존 없음. 연도 경과 시 차기 연도 공휴일을 시드해 갱신해야 하며(운영 작업), Google 공휴일 자동 동기화는 후속.
...
- leave service가 신청·수정 시 대상 기간의 공휴일 Set을 조회해 `calculateLeaveDays`에 주입한다. `Holiday` 테이블은 시드로 채워져 있어 **결정적**이다(휘발성 캐시 아님).
- 시드 범위(현재+익년) **밖 날짜**는 `Holiday` 행이 없어 그 구간은 주말만 제외된다 — 연말에 차기 연도 공휴일을 시드해야 정확도가 유지된다(운영 작업, §12 후속).
...
- **Holiday 연도 갱신·자동 동기화**: 현재는 현재+익년 정적 시드. 연말 차기 연도 공휴일 시드 갱신은 운영 작업이며, Google 공휴일 캘린더 → `Holiday` 자동 동기화 admin 액션은 후속. `calendar`의 `Holiday` 리더 전환도 후속.
