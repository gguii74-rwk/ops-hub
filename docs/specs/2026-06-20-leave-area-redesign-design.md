...
- 시점: 신청(관리자 통지)/승인/반려(신청자 통지)/관리자 직접입력(`sendNotification` 체크 시 신청자 통지).
- 구현(durable·idempotent — 적대검증 finding 반영): 연차 트랜잭션 **내부**에서 발송 대상 레코드(`MailDelivery`/outbox)를 `PENDING`으로 생성하고, **`leaveRequestId` + `eventType`(REQUESTED/APPROVED/REJECTED/ADMIN_CREATED)을 idempotency key**로 둔다. 실제 SMTP 발송과 상태 확정(SENT/FAILED)은 커밋 후 background worker가 수행. 이렇게 하면 커밋 후 프로세스가 죽어도 레코드가 남아 재시도 가능하고, 같은 이벤트의 중복 발송을 막는다.
- 기존 workflow `MailDelivery`가 `taskId`/`step` 키 기반이면 leave 식별자(`leaveRequestId`+`eventType`)를 수용하도록 확장하거나 leave 전용 outbox를 둔다(구현 계획에서 확정 — 15절).
- 발송 실패가 신청/승인 자체를 실패시키지 않는다(연차 도메인 불변식). 직접입력은 `sendNotification` 체크 시에만.
- 본문: 원본 `email.service.ts` 템플릿을 참고하되 ops-hub 발송기 형식에 맞춤.
...
- **메일 outbox(8절):** `MailDelivery` 모델을 leave 이벤트 키로 확장하면 추가 migration이 생길 수 있다(또는 leave 전용 outbox). 구현 계획에서 확정.
...
- `MailDelivery` 모델을 leave 이벤트 키(`leaveRequestId`+`eventType`)로 확장할지, leave 전용 outbox를 둘지 — 구현 계획에서 확정(8·10절).
