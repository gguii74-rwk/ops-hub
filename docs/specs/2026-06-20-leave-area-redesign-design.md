...
- 확인 모달 후 DELETE. **권한 `leave.request:delete`.** usedDays 재계산 transaction.
...
**worker 계약(lease 기반 — finding):** `MailDelivery`에 `lockedUntil DateTime?`·`workerId String?` 추가. 발송 후보 = `PENDING` OR (`FAILED && attempts < N`) OR (**`SENDING && lockedUntil < now`** — claim 직후 크래시한 stale 행 회수). worker는 **atomic 조건부 update**로 claim한다: 상태·`lockedUntil` 조건이 맞을 때만 `SENDING` + `lockedUntil = now + lease` + `workerId` 설정(영향 행 0이면 타 worker 선점 → skip). 발송 성공 → `SENT`(+`providerMessageId`·`sentAt`, `lockedUntil=null`), 실패 → `FAILED`(+`errorMessage`·`attempts++`, `lockedUntil=null`). 이렇게 하면 claim 후 크래시해도 lease 만료 뒤 재집어 **유실되지 않는다**.
...
- **메일 outbox(8절) — 확정:** `MailDelivery`(workflows 스키마) 필드 추가 migration — `leaveRequestId String?`, `eventType String?`, `attempts Int @default(0)`, `lockedUntil DateTime?`, `workerId String?`, `@@unique([leaveRequestId, eventType])`. `MailDeliveryStatus`에 `PENDING` 추가(현재 SENDING/SENT/FAILED).
