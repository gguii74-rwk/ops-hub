...
- 스키마 컬럼 추가 — 필요한 필드는 이미 존재(아래 5·10절).
...
- API: 기존 `GET /api/leave/requests`(본인) + `GET /api/admin/leave/requests`(전체, 필터 파라미터 추가 필요 시 확장).
...
- 일반: 본인 + **같은 부서** 직원. 권한 없는 타인의 사유·세부는 마스킹(제목만). 날짜 클릭 → `/leave/request?date=` 이동.
- 관리자: **부서/사용자 필터** + "+ 연차 입력"(직접입력 모달, 날짜 prefill).
- 통합 캘린더(`/calendar`)와 독립.

## 7. 관리자 모달 (직접입력/수정/삭제)

원본 `CreateLeaveModal`/`EditLeaveModal` 포팅. 백엔드는 이미 존재(`adminCreateLeaveSchema`+`createLeaveRequestByAdmin`, `updateLeaveSchema`+PATCH, DELETE).
...
- 구현: workflows의 메일 인프라(`MailDelivery` 모델·발송기) 재사용. **background 발송**(업무 트랜잭션 성공과 분리) + 이력 기록. 발송 실패가 신청/승인 자체를 실패시키지 않는다(연차 도메인 불변식).
...
- **스키마 변경 없음.** `LeaveRequest.leaveSubType`/`quarterStartTime`/`adminActionNote`/`createdByAdminId/At`/`modifiedByAdminId/At`/`isCarriedOver`, `User.department`, `LeaveAllocation*` 모두 존재.
- 변경은 **권한 seed 2종 추가**(코드/데이터)뿐 — Prisma migration 불필요.
- 시간대 6종은 **코드 상수**(DB 아님).
