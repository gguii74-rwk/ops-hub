...
- 삭제-발송 race(finding): claim 후 provider 성공 전 soft-delete 시 worker 조건부 finalize가 0행이라 `CANCELLED`를 `SENT`로 덮지 않음.
...
- 메일 worker finalize는 **조건부 update**(`status=SENDING AND workerId` 일치)로 `CANCELLED` race 안전.
