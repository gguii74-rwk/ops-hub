...
### 공통 — 대상(target) 권한 재검증 (finding)

- 사용자 목록 API뿐 아니라 **모든 admin 변경 service(직접입력 create / 수정 update / 삭제 delete)가 actor의 대상 스코프를 서버에서 재검증**한다 — 드롭다운을 우회해 위조 `userId`·request id를 POST해도 거부. 대상 사용자의 부서가 null·모호하면 **fail-closed**. coarse 권한(`approval:approve`/`request:update`/`request:delete`) 보유만으로 임의 대상에 작용하지 못하도록 공유 target-authorization 헬퍼로 강제한다.
...
- 대상 스코프(finding): 위조 out-of-scope `userId`/request id로 admin create/update/delete 시도 시 fail-closed 거부.
...
- 대원칙: 원본 기능을 그대로 포팅(단, 삭제 감사·권한 경계는 ops-hub 원칙으로 강화).
