...
시점·수신자: 신청(관리자 통지) / 승인·반려(신청자 통지) / 관리자 직접입력(`sendNotification` 체크 시 신청자 통지). **관리자 통지 수신자는 permission 기반(결정)** — `leave.approval:view` 유효 보유자(systemRole이 MANAGER라도 승인권한 없으면 제외; 메일로 권한경계 우회 차단). **전 `ACTIVE` 사용자에 `hasPermission` 평가**로 확정 — role/override로 권한 받은 MEMBER도 포함(systemRole prefilter는 부여자 누락→알림 유실이라 안 씀, finding). 사내 규모(수십명) 전제.

**메일 본문 이스케이프(finding):** `reason`·`rejectionReason`·신청자 이름 등 사용자/관리자 입력을 HTML 본문에 넣을 때 반드시 HTML-encode한다(저장형 HTML 인젝션 차단). subject는 HTML이 아니므로 이스케이프 대상 아님.
...
- 상태: `MailDeliveryStatus`에 **`PENDING`·`CANCELLED` 추가**(현재 SENDING/SENT/FAILED). 전이 `PENDING → SENDING → SENT | FAILED | CANCELLED`, soft-delete 시 `PENDING|FAILED|stale SENDING → CANCELLED`(terminal; **active SENDING은 제외 — 결정 A**). worker는 `CANCELLED`·`SENT`를 후보로 보지 않는다.
...
**worker 계약(lease 기반 + leave 스코프 — finding):** `MailDelivery`에 `lockedUntil DateTime?`·`workerId String?` 추가. **발송 후보는 leave 행으로 한정** — `leaveRequestId IS NOT NULL AND eventType IS NOT NULL AND ( PENDING OR (FAILED && attempts < N) OR (SENDING && lockedUntil < now && attempts < N) )`. 기존 workflow 전달 행(`leaveRequestId IS NULL`, `taskId` 기반)은 **절대 집지 않는다**(공유 테이블 오염 방지). worker는 **atomic 조건부 update**로 claim한다: 후보 조건이 맞을 때만 `SENDING` + `lockedUntil = now + lease` + `workerId` + `attempts++` 설정(영향 행 0이면 타 worker 선점 → skip).

- **dead-letter(finding):** claim이 `attempts`를 먼저 올리므로 N번째 claim 후 크래시하면 stale `SENDING && lockedUntil < now && attempts >= N`으로 남아 후보에서 빠진다 → 영구 표류·silent 유실. drain 시작 시 이 행들을 **발송하지 않고 `FAILED`로 종결**(dead-letter)한다.
- **발송 직전 재확인(pre-send):** claim 후 `leaveRequest.deletedAt`를 확인 — 삭제됐으면 발송하지 않고 `CANCELLED`로 finalize("claim 후 삭제" 윈도 차단).
- **조건부 finalize**(`WHERE status=SENDING AND workerId=자신`): 성공 → `SENT`(+`providerMessageId`·`sentAt`·`lockedUntil=null`), SMTP 실패 → `FAILED`(+`errorMessage`·`lockedUntil=null`), 발송 전 삭제 감지 → `CANCELLED`. **영향 0행이면** 타 worker 선점/terminal → 결과 폐기(덮어쓰지 않음).
- **삭제-발송 semantics(결정 A):** soft-delete cancel은 `PENDING/FAILED/stale SENDING`만 `CANCELLED`로 하고 **active SENDING(lease 유효)은 건드리지 않는다** — worker가 정직하게 finalize(`SENT`/`FAILED`). 발송이 이미 진행 중인 잔여 윈도에서 삭제되면 메일이 나가고 `SENT`로 정직 기록될 수 있다(요청은 `deletedAt`로 별도 감사). 실제 발송을 `CANCELLED`로 지워 감사를 왜곡하지 않는다(at-least-once·"메일이 연차 tx를 막지 않음" 결정과 일치). claim 후 크래시해도 lease 만료 뒤 재집어 유실되지 않는다.

**전달 보장(사용자 결정 — at-least-once, 누락 방지):** stale `SENDING` 재집기(provider 수락 후 SENT 마킹 전 크래시)는 이미 보낸 메일을 재전송할 수 있다. 이를 통제하기 위해: **stale reclaim도 `attempts++` 하고 상한 `N`을 적용**(무한 재발송 방지; 상한 초과 stale는 dead-letter `FAILED` 확정 후 수동 확인 — 그래서 `SENDING` reclaim도 `attempts < N`으로 게이트). "이벤트당 행 1개"(`@@unique`)는 우리 측 중복 트리거를 막는다. **드문 중복 발송은 허용**(연차 알림은 중복이 누락보다 덜 해롭다); exactly-once는 목표가 아니다. **주의:** 현재 발송기 `sendMail`은 idempotency/message-key 인자가 없고 `providerMessageId`는 발송 **후** 반환되므로 provider 단 재전송 억제는 **하지 못한다**(과거 초안의 "provider idempotency key" 표현은 미구현 — 감사용 기록일 뿐). 향후 transport가 키를 지원하면 `leaveRequestId:eventType`를 `MailMessage`에 실어 억제할 수 있다.
...
- 메일 at-least-once(결정): crash-after-provider-accept 시 재전송하되 `attempts` cap 적용, `SENDING` reclaim도 `attempts < N` 게이트. 드문 중복 허용(provider 단 idempotency 억제는 미구현 — `sendMail`에 키 인자 없음; `providerMessageId`는 감사용).
- 메일 취소(finding): soft-delete 시 `PENDING`/`FAILED`/stale `SENDING` outbox만 `CANCELLED`로 전이되고(active SENDING은 제외) worker가 발송하지 않음.
- 메일 dead-letter(finding): N번째 claim 후 크래시한 stale `SENDING`(`attempts >= N`)이 drain 시작 시 `FAILED`로 종결돼 영구 표류·silent 유실되지 않음.
- 메일 발송 직전 재확인(결정 A): claim 후 요청이 soft-delete돼 있으면 발송하지 않고 `CANCELLED`로 finalize, `sendMail` 미호출.
- 메일 수신자 0명(finding): 승인권한자 0명이어도 REQUESTED outbox 행은 적재되고 worker가 "수신자 없음" `FAILED`로 종결(운영자 가시).
...
- 메일 전달 = **at-least-once(누락 방지)** + attempts cap + dead-letter. 중복 억제는 "이벤트당 행 1개"(`@@unique`)까지 — provider 단 idempotency는 미구현(드문 중복 허용, exactly-once 비목표).
- 캘린더 가시성: 일반 사용자는 타인 휴가를 **APPROVED만**(동료 PENDING/REJECTED 비노출), 본인은 전체. cross-user pending/rejected는 status/admin 권한.
- 메일 삭제-발송 = **결정 A(발송 중은 정직하게 마무리)**: soft-delete cancel은 `PENDING/FAILED/stale SENDING`만 `CANCELLED`로 하고 **active SENDING은 건드리지 않음**(worker가 `SENT/FAILED`로 정직 finalize). 발송 직전 `deletedAt` 재확인으로 "claim 후 삭제"는 미발송(`CANCELLED`); 발송 진행 중 삭제 잔여 윈도는 `SENT`로 정직 기록(요청은 `deletedAt` 감사). 실제 발송을 `CANCELLED`로 지워 감사를 왜곡하지 않음. — 사용자 결정.
- 메일 dead-letter(plan finding): N번째 claim 후 크래시한 stale `SENDING`(`attempts >= N`)은 drain 시작 시 `FAILED`로 종결(영구 표류 방지).
- 메일 수신자 0명이어도 REQUESTED outbox 행은 **항상 적재**(durable; worker가 `FAILED` 종결로 운영자에 노출). — plan finding.
