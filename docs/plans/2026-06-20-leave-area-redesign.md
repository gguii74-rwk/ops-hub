...
- **수신자(permission 기반 — 결정):** `getLeaveAdminRecipients()` = **`leave.approval:view` 유효 보유자 전원**의 email 배열(REQUESTED용). **전 `ACTIVE` 사용자**에 `hasPermission(id,"leave.approval","view")`를 평가해 확정 — role/override로 권한 받은 MEMBER도 포함, 승인권한 없는 MANAGER 제외(systemRole prefilter는 부여자 누락→알림 유실이라 쓰지 않음, finding). 사내 규모(수십명) 전제. **수신자 0명이어도 REQUESTED 행은 항상 적재**(durable; worker가 "수신자 없음" FAILED로 종결해 운영자가 누락을 본다 — finding). 신청자 통지(APPROVED/REJECTED/ADMIN_CREATED)는 해당 `LeaveRequest.user.email`.
...
- **dead-letter(drain 시작 시):** stale `SENDING AND lockedUntil < now AND attempts >= N`은 후보에서 빠져 영구 표류하므로(claim이 attempts를 먼저 올림 → N번째 claim 후 크래시) 발송하지 않고 **`FAILED`로 종결**(운영자 가시·재시도 종료, finding).
- **claim(atomic 조건부 update):** 위 후보 조건에 맞을 때만 `status=SENDING, lockedUntil=now+lease, workerId=self, attempts: { increment: 1 }`. 영향 0행이면 선점됨 → skip. claim 결과로 `leaveRequestId`도 반환(발송 직전 재확인용).
...
- **구동(하이브리드 + 감독):** 연차 작업 라우트가 커밋 성공 후 `triggerLeaveMailDrain()`(= `drainLeaveMailOutbox().catch(log)` 래퍼 — await 안 함, 실패가 응답을 막지 않고 unhandled rejection도 안 됨) + **시스템 스케줄러가 1~5분마다 `POST /api/leave/mail/drain`(x-drain-token) 호출이 backstop**(누락 보충). 이 cron 등록은 at-least-once의 필수 조건이라 **배포 시 등록·검증**하고, stale `PENDING`/dead-letter `FAILED` 행을 모니터링한다(Task 03 §6b).
