...
| P2′ | (P2 3·4회째 재출현, R4·R5) cutover가 코드/CI 강제 없는 수동 preflight 의존 | high | **DUPLICATE(사용자 ACCEPTED)** | P2와 동일 fingerprint. 사용자가 R3 batch에서 **B1(수동 preflight)** 을 명시 선택(저장소 배포가 전부 수동인 맥락). 사용자 결정이 우선 — 재오픈 안 함(반복 → 판정으로 종결). enforced 스크립트는 follow-up 후보로만 기록. |
| P5 | `Number(process.env.SMTP_PORT ?? 587)`에서 `SMTP_PORT=""`→`Number("")===0`(finite)이라 NaN 가드 통과 → **port 0 발송 시도**, 상태는 port 무관이라 "정상"(숨은 장애) | high | **FIXED** | R5 지적(정확·내 버그). task-03 — `trim()` 후 빈문자열/비정수/범위밖(0·>65535)은 587로. §7에 `''`·`'   '`·`'0'`·`'70000'` 회귀 테스트 추가. |
| P6 | spec D5("Google configured=활성 소스 카운트")가 D6/§5.3("PR-A는 calendarIds 유지")와 모순 → SSOT라 구현자가 D5 따르면 PR-B 의존 조기 도입 | high | **FIXED** | R5 지적(정확·스펙 자체 모순). spec §5.3 D5 절을 **PR-A=calendarIds / PR-B=소스 카운트**로 명시 분리. task-05 google 테스트가 이미 calendarIds 기반 판정을 단언(소스 카운트 아님 — P6 권고 테스트 충족). |

**추세(미판정 blocking score, high=3·medium=1)**: R1=3(P1) → R2=4(P2+P3) → R3 재출현 → **사용자 batch 판정**(B1·A2) → R4=3(P4 SSOT 모순+P2 3회째) → P4 FIXED·P2′ DUP → R5=6(P5 port0 신규 버그 + P6 D5/D6 모순 + P2 4회째) → **P5·P6 FIXED·P2 DUP → 0**. **iteration 5 = max 도달**으로 종료. 최종 disposition: **FIXED** P1·P3·P5·P6 + P4(SSOT reconcile), **ACCEPTED(사용자 B1)** P2. 미판정 blocking 0. (impl 단계 review-loop가 §7 회귀 테스트로 재검증.)
