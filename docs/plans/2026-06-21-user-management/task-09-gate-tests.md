...
2. **본 task 파일만 실행(빠른 피드백)**
   ...
   - 열거 메타검사: `enumerateApiRoutes()` 결과가 비어 있지 않고, **모든 라우트가 `publicAllowlist`∪`exercisedRoutes`∪`knownProtected` 세 분류 중 하나로 등록**됨(미분류 시 경로를 출력하며 실패). `/api/auth/verify`가 `exercisedRoutes`에 포함됨.
   ...
   - D13: ⓐ~ⓕ 각 경로 EscalationError·MinAvailabilityError 전파 + repository 미호출, OWNER 대조군 허용. **finding C**: 위임 admin의 특권 역할 **제거**(목록 누락)·systemRole **강등**도 EscalationError·repository 미호출.
