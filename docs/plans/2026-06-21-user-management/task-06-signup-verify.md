...
**Create**
- `src/modules/admin/users/rate-limit.ts` — **S10 공유 상수(`VERIFY_TOKEN_TTL_MS`/`SIGNUP_IP_LIMIT`/`SIGNUP_EMAIL_LIMIT`/`RESEND_COOLDOWN_MS`/`RATE_WINDOW_MS`/`PENDING_UNVERIFIED_CAP`) 정의·export** + D18 RateBucket 원자적·사전 강제 + per-IP/per-email + `PENDING_UNVERIFIED_CAP` 전역 상한 + IP 추출 (상수·강제 유틸이 같은 파일. task-09 테스트도 여기서 import)
...
- **03** (repository): `createPendingSignup`/`setPasswordViaToken`/`refreshVerifyToken`(S6) + `enqueueUserMail`(S8) + `triggerLeaveMailDrain`(공통 트리거, S8). 이 task는 repo 함수의 **호출·반환 계약만** 의존(전부 mock).
...
별도 실패 테스트 불필요(값 상수). Step 2 테스트가 import로 소비. Step 5에서 같은 파일에 강제 유틸(`extractClientIp`/`enforceRateLimit`/`enforceResendCooldown`/`enforcePendingCap`)을 이어 작성한다.
...
`tests/modules/admin/users/rate-limit.test.ts` — leave repositories.test.ts의 `vi.hoisted` fake-db 패턴. `RateBucket` upsert+increment를 `$transaction`으로 직렬화하므로, fake `$transaction`은 cb에 fake db를 패스스루한다.
...
const h = vi.hoisted(() => {
  const db = {
    rateBucket: { findUnique: vi.fn(), upsert: vi.fn(), update: vi.fn(), create: vi.fn() },
    user: { count: vi.fn() },
  };
...
import { extractClientIp, enforceRateLimit, enforcePendingCap, enforceResendCooldown, SIGNUP_IP_LIMIT, SIGNUP_EMAIL_LIMIT, RATE_WINDOW_MS, RESEND_COOLDOWN_MS } from "@/modules/admin/users/rate-limit";
...
describe("enforceRateLimit (원자적·사전·윈도우 리셋)", () => {
  const now = new Date("2026-06-21T00:00:00Z");
  it("첫 시도면 count=1로 버킷 생성·통과", async () => {
  ...
  it("윈도우 내 한도 초과면 RateLimitError(429) — increment 결과가 limit 초과", async () => {
    // 기존 버킷이 윈도우 내·이미 limit. upsert increment 후 count>limit.
    h.db.rateBucket.findUnique.mockResolvedValue({ count: SIGNUP_EMAIL_LIMIT, windowStartedAt: now });
    h.db.rateBucket.update.mockResolvedValue({ count: SIGNUP_EMAIL_LIMIT + 1, windowStartedAt: now });
    await expect(enforceRateLimit("signup:email", "a@x.com", SIGNUP_EMAIL_LIMIT, now)).rejects.toBeInstanceOf(RateLimitError);
  });
  it("윈도우 만료면 count=1·windowStartedAt=now로 리셋·통과", async () => {
    const stale = new Date(now.getTime() - RATE_WINDOW_MS - 1);
    h.db.rateBucket.findUnique.mockResolvedValue({ count: 99, windowStartedAt: stale });
    h.db.rateBucket.update.mockResolvedValue({ count: 1, windowStartedAt: now });
    await expect(enforceRateLimit("signup:ip", "203.0.113.7", SIGNUP_IP_LIMIT, now)).resolves.toBeUndefined();
    expect(h.db.rateBucket.update).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ count: 1, windowStartedAt: now }),
    }));
  });
});

describe("enforceResendCooldown (per-email 쿨다운)", () => {
  const now = new Date("2026-06-21T00:00:00Z");
...
describe("enforcePendingCap (전역 상한)", () => {
  it("미검증 PENDING이 상한 미만이면 통과", async () => {
    h.db.user.count.mockResolvedValue(10);
    await expect(enforcePendingCap()).resolves.toBeUndefined();
    expect(h.db.user.count).toHaveBeenCalledWith({ where: { status: "PENDING", emailVerifiedAt: null } });
  });
  it("상한 도달이면 RateLimitError(429)", async () => {
    h.db.user.count.mockResolvedValue(200);
    await expect(enforcePendingCap()).rejects.toBeInstanceOf(RateLimitError);
  });
});
```
...
`src/modules/admin/users/rate-limit.ts`에 Step 1 상수 블록 **아래로 이어 작성**한다(상수·강제 유틸 동일 파일 — `./policy` import 없음). D18은 **원자적·사전(pre-write)**: 버킷 증가가 한도를 넘으면 User/MailDelivery 생성 전에 `RateLimitError`. 동일 윈도우 내 동시 요청 race는 `$transaction`(직렬화)으로 좁힌다.
...
// 원자적·사전 카운터: 윈도우 내면 increment, 만료면 리셋(count=1). increment 후 count>limit이면 거부.
// $transaction으로 read→write를 직렬화해 동시 요청이 한도를 넘겨 통과하는 race를 좁힌다(다중 인스턴스 안전 — DB-backed).
export async function enforceRateLimit(
  ...
  const exceeded = await prisma.$transaction(async (tx) => {
    const bucket = await tx.rateBucket.findUnique({ where: { scope_key: { scope, key } } });
    if (!bucket) {
    ...
    const windowExpired = bucket.windowStartedAt.getTime() + RATE_WINDOW_MS < now.getTime();
    if (windowExpired) {
      await tx.rateBucket.update({ where: { scope_key: { scope, key } }, data: { count: 1, windowStartedAt: now } });
      return false;
    }
    const updated = await tx.rateBucket.update({
      where: { scope_key: { scope, key } },
      data: { count: { increment: 1 } },
    });
    return updated.count > limit;
  });
  if (exceeded) throw new RateLimitError("요청이 너무 많습니다. 잠시 후 다시 시도해 주세요.");
}

// per-email 재발송 쿨다운: 마지막 갱신(updatedAt)으로부터 RESEND_COOLDOWN_MS 이내 재발송 거부. 사전 검사 후 타임스탬프 갱신.
...
// 전역 미검증 PENDING 상한(bounded creation) — 봇이 검증 안 한 PENDING으로 DB를 채우는 것 차단. 사전 검사.
export async function enforcePendingCap(): Promise<void> {
  const pending = await prisma.user.count({ where: { status: "PENDING", emailVerifiedAt: null } });
  if (pending >= PENDING_UNVERIFIED_CAP) {
    throw new RateLimitError("현재 신규 가입을 받을 수 없습니다. 잠시 후 다시 시도해 주세요.");
  }
}
```

> `scope_key`는 `RateBucket`의 `@@unique([scope, key])` 복합 unique 입력 이름(S1). Prisma는 `where: { scope_key: { scope, key } }`로 노출한다. `bucket.updatedAt`은 `@updatedAt` 자동 필드(S1).
...
git commit -m "feat(user-mgmt): D18 레이트리밋 원자적·사전 강제(per-IP/email·쿨다운·PENDING 상한)(task-06)"
```
...
`tests/app/auth-signup-route.test.ts` — leave route 테스트 패턴(`vi.mock` repo/rate-limit/token/mail). **핵심 단언: 한도 초과 시 createPendingSignup·enqueueUserMail·triggerLeaveMailDrain 미호출 + 429.**

```ts
import { describe, it, expect, vi, beforeEach } from "vitest";

const m = vi.hoisted(() => ({
  enforceRateLimit: vi.fn(), enforcePendingCap: vi.fn(), extractClientIp: vi.fn(() => "1.2.3.4"),
  createPendingSignup: vi.fn(), enqueueUserMail: vi.fn(), triggerLeaveMailDrain: vi.fn(),
  generateVerifyToken: vi.fn(() => "plain-token"), hashToken: vi.fn((t: string) => `hash:${t}`),
  txRun: vi.fn(async (cb: (tx: unknown) => unknown) => cb({})),
}));

vi.mock("@/modules/admin/users/rate-limit", () => ({
  enforceRateLimit: m.enforceRateLimit, enforcePendingCap: m.enforcePendingCap, extractClientIp: m.extractClientIp,
}));
vi.mock("@/modules/admin/users/repositories", () => ({ createPendingSignup: m.createPendingSignup }));
vi.mock("@/modules/admin/users/repositories/mail", () => ({ enqueueUserMail: m.enqueueUserMail }));
...
vi.mock("@/lib/prisma", () => ({ prisma: { $transaction: m.txRun } }));
...
beforeEach(() => {
  ...
  m.txRun.mockImplementation(async (cb: (tx: unknown) => unknown) => cb({}));
});

describe("POST /api/auth/signup", () => {
  it("정상: PENDING 생성 + 검증 메일 enqueue + drain 트리거 + 중립 200/202", async () => {
    m.enforceRateLimit.mockResolvedValue(undefined);
    m.enforcePendingCap.mockResolvedValue(undefined);
    m.createPendingSignup.mockResolvedValue({ id: "u-self" });
    const res = await POST(body(valid));
    expect([200, 202]).toContain(res.status);
    expect(m.createPendingSignup).toHaveBeenCalledWith(expect.objectContaining({ email: "self@x.com", tokenHash: "hash:plain-token" }));
    expect(m.enqueueUserMail).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ eventType: "VERIFY_EMAIL", recipients: ["self@x.com"] }));
    expect(m.triggerLeaveMailDrain).toHaveBeenCalled();
  });

  it("D18 한도 초과: createPendingSignup·메일·drain 미호출 + 429 (pre-write)", async () => {
    ...
    expect(m.enqueueUserMail).not.toHaveBeenCalled();
    expect(m.triggerLeaveMailDrain).not.toHaveBeenCalled();
  });

  it("PENDING 상한 초과: 행 미생성 + 429", async () => {
    const { RateLimitError } = await import("@/modules/admin/users/errors");
    m.enforceRateLimit.mockResolvedValue(undefined);
    m.enforcePendingCap.mockRejectedValueOnce(new RateLimitError("cap"));
    const res = await POST(body(valid));
    expect(res.status).toBe(429);
    expect(m.createPendingSignup).not.toHaveBeenCalled();
  });

  it("중복 이메일(UserConflictError): 중립 200/202(열거 방지) — 메일·drain은 미호출", async () => {
...
`src/app/api/auth/signup/route.ts`. D18 사전 강제 → 토큰 생성 → `createPendingSignup` → 검증 메일 enqueue(트랜잭션 내) → drain. **중복(UserConflictError)은 중립 응답으로 흡수**(이메일 열거 방지, D10).

```ts
import { NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import { signupSchema } from "@/modules/admin/users/validations/signup";
import { extractClientIp, enforceRateLimit, enforcePendingCap, SIGNUP_IP_LIMIT, SIGNUP_EMAIL_LIMIT, VERIFY_TOKEN_TTL_MS } from "@/modules/admin/users/rate-limit";
import { generateVerifyToken, hashToken } from "@/modules/admin/users/token";
import { createPendingSignup } from "@/modules/admin/users/repositories";
import { enqueueUserMail } from "@/modules/admin/users/repositories/mail";
...
export async function POST(req: Request) {
  ...
  try {
    // ── D18 사전 강제(원자적·pre-write): 한도 초과 시 User/MailDelivery 행 생성 전 429 ──
    ...
    await enforcePendingCap();

    // 토큰: 평문은 메일 링크에만, DB엔 해시.
    ...
    // 검증 메일 enqueue(공통 MailDelivery, leaveRequestId=null). 평문 토큰을 요청 origin 기준 절대 링크로.
    const origin = new URL(req.url).origin;
    const link = `${origin}/verify-email?token=${plainToken}`;
    const { subject, bodyHtml } = buildVerifyEmailMail(link);
    await prisma.$transaction(async (tx) => {
      await enqueueUserMail(tx, { eventType: "VERIFY_EMAIL", recipients: [email], subject, bodyHtml });
    });
...
const m = vi.hoisted(() => ({
  enforceResendCooldown: vi.fn(), refreshVerifyToken: vi.fn(), enqueueUserMail: vi.fn(),
...
beforeEach(() => { vi.clearAllMocks(); m.txRun.mockImplementation(async (cb: (tx: unknown) => unknown) => cb({})); });

describe("POST /api/auth/resend-verification", () => {
  it("미검증 PENDING 존재: 새 토큰·메일 enqueue·drain + 중립 200/202", async () => {
    ...
    expect([200, 202]).toContain(res.status);
    expect(m.refreshVerifyToken).toHaveBeenCalledWith("a@x.com", "hash:plain", expect.any(Date));
    expect(m.enqueueUserMail).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ eventType: "VERIFY_EMAIL" }));
    expect(m.triggerLeaveMailDrain).toHaveBeenCalled();
  });
  it("존재하지 않는 이메일(refreshVerifyToken null): 동일 중립 응답·메일 미발송(열거 방지)", async () => {
    m.enforceResendCooldown.mockResolvedValue(undefined);
...
`src/app/api/auth/resend-verification/route.ts`. 쿨다운(사전) → `refreshVerifyToken`(미검증 PENDING만 매칭, 없으면 null) → 있으면 메일 enqueue + drain. 존재 여부와 무관하게 동일 중립 응답.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 미검증 PENDING만 토큰 갱신. 대상 없으면 null → 메일 없이 동일 중립 응답(열거 방지).
    const target = await refreshVerifyToken(email, tokenHash, tokenExpiresAt);
    if (target) {
      const origin = new URL(req.url).origin;
      const link = `${origin}/verify-email?token=${plainToken}`;
      const { subject, bodyHtml } = buildVerifyEmailMail(link);
...
- `npm test -- tests/modules/admin/users/rate-limit` → PASS (윈도우 리셋·한도 초과 429·쿨다운·전역 상한).
- `npm test -- tests/modules/admin/users/signup-validation` → PASS (비번 없는 signupSchema·12자+ setPasswordSchema).
- `npm test -- tests/app/auth-signup-route` → PASS. 특히 **한도 초과/PENDING 상한 케이스에서 `createPendingSignup`·`enqueueUserMail`·`triggerLeaveMailDrain`이 호출되지 않고 429**(`expect(...).not.toHaveBeenCalled()` 그린).
...
- **Don't 레이트리밋·상한을 행 생성 '뒤'에 검사하지 마라(pre-write 필수).** Reason: D18은 **원자적·사전** — `enforceRateLimit`(per-IP·per-email)·`enforcePendingCap`을 `createPendingSignup`/메일 enqueue **전에** 호출해, 한도 초과면 User·MailDelivery 행이 한 줄도 안 생기고 429. signup-route 테스트의 `not.toHaveBeenCalled()`가 회귀 가드다. resend도 `enforceResendCooldown`을 `refreshVerifyToken` 전에.
...
- **Don't repository를 우회해 직접 prisma로 User를 쓰지 마라.** Reason: User 생성·토큰 소비·갱신은 S6 repository 함수(`createPendingSignup`/`setPasswordViaToken`/`refreshVerifyToken`)만 호출한다(만료 PENDING 교체·status-CAS·원자적 토큰 매칭이 거기 캡슐화됨). **직접 prisma 쓰기는 `RateBucket`(rate-limit.ts)·검증메일 enqueue tx·GET 토큰 유효성 조회로 한정**.
...
- **Don't `RateBucket` 동시 증가를 단순 read-then-write로 하지 마라.** Reason: 동시 요청이 같은 윈도우에서 한도를 넘겨 통과할 수 있다. `enforce*`는 `prisma.$transaction`으로 read→increment를 묶어 직렬화하고, DB-backed라 다중 인스턴스에서도 안전(S10·spec D18 "DB-backed durable").
