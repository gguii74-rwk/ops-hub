...
- Create: `src/app/api/admin/users/[id]/route.ts` — `GET`(`:view`), `PATCH`(`:update`, **빈 patch는 400**)
- Create: `src/app/api/admin/users/[id]/status/route.ts` — `POST`(`admin.users:update`) — 상태 토글 전용(D14 세션무효화, finding E)
...
- Create (Test): `tests/app/api/admin/users/id-route.test.ts` — `GET`/`PATCH /api/admin/users/[id]`(빈 patch 400 포함)
- Create (Test): `tests/app/api/admin/users/status-route.test.ts` — `POST /api/admin/users/[id]/status`(상태 전이 위임·세션무효화)
...
## Prep
- entrypoint §Shared Contracts: **S4**(도메인 에러 종류 + 라우트 매핑 표 — `ForbiddenError`/`EscalationError`→403, `UserConflictError`/`MinAvailabilityError`→409, `UserValidationError`/`TokenError`→400, `RateLimitError`→429), **S5**(`ActorContext` = `{userId, isOwner, permissionKeys}`), **S7**(이 task가 호출할 service 함수 시그니처 전부 + validation 스키마 목록), **S11**(finding E — 상태 토글 전용 라우트 `POST .../[id]/status`로 `setUserStatus` 호출·`PATCH /[id]` 빈 patch 400 거부).
...
describe("_shared buildActorCtx (S5·finding 3 — isOwner는 summary 단일 권위)", () => {
  it("summary.isOwner=true면 isOwner=true, permissionKeys=Set(keys)", () => {
    const ctx = buildActorCtx({ id: "o1" } as any, { keys: ["admin.users:view"], isOwner: true });
  ...
  it("summary.isOwner=false면 isOwner=false — stale session.user.systemRole(OWNER)은 무시(finding 3)", () => {
    const ctx = buildActorCtx({ id: "a1", systemRole: "OWNER" } as any, { keys: [], isOwner: false });
...
### 5b. 실패 테스트 — `/api/admin/users/[id]/status` (상태 토글 전용)

`tests/app/api/admin/users/status-route.test.ts`. finding E 전용 라우트: `POST .../[id]/status`가 body `{status:"ACTIVE"|"DISABLED"}`를 `statusSchema`로 검증하고 `admin.users:update` 권한키 검사 후 `setUserStatus(ctx, id, status)`에 위임한다. service가 D14 세션무효화(`sessionInvalidatedAt`)·재활성 분기를 담당하므로 라우트는 위임만 한다. service가 `MinAvailabilityError`(마지막 관리자 비활성화)면 409, `EscalationError`(특권 대상/자가)면 403.
...
```
npm test -- tests/app/api/admin/users/status-route   # expect FAIL (라우트 미존재)
```

### 5c. 최소 구현 — `/api/admin/users/[id]/status` route

`src/app/api/admin/users/[id]/status/route.ts`. body `{status}`를 인라인 zod(`z.enum(["ACTIVE","DISABLED"])`)로 검증하고 `setUserStatus`에 위임한다. status 전이 스키마는 admin validations(S7)에 두지 않고 이 라우트 전용으로 인라인한다(2값 enum 하나뿐 — 별도 파일 과함). REJECTED→ACTIVE 재활성도 service `setUserStatus`가 동형 분기로 흡수(S6 `reactivateRejectedTx`/`setStatusTx`).
...
> finding E: 상태 변경을 `PATCH /[id]`(프로필·systemRole 전용)에 섞지 않고 이 전용 라우트로 분리한다. `PATCH`는 `updateUserSchema`에 `status`가 없어 zod가 strip → 빈 patch가 되므로 step 5에서 400으로 막았고, 실제 상태 전이(세션무효화 동반)는 여기로만 들어온다. `setUserStatus`의 D14 세션무효화(`sessionInvalidatedAt`)·anti-escalation·최소가용성은 service(task-04)·repository(task-03)가 강제하며, 라우트는 위임만 한다.

```
npm test -- tests/app/api/admin/users/status-route   # expect PASS
```
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const summary = await getPermissionSummary(session.user.id);
    await approveUser(buildActorCtx(session.user, summary), id, parsed.data);
    return NextResponse.json({ ok: true });
  } catch (error) {
    return mapError(error);
  }
...
npm test -- tests/app/api/admin/users   # 5개 파일 전부 PASS (route·id-route·status-route·approve-reject·mutations)
npm run lint        # 0 errors (boundaries: app→{kernel,lib,module} 허용이라 service/_shared import 통과)
```

기대:
- `npm test -- tests/app/api/admin/users` → `Test Files  5 passed`, 전 테스트 green.
...
- `PATCH /[id]`는 알려진 필드 0개인 빈 patch를 400으로 거부하고(`status` 등 unknown 키 strip 방지), 상태 전이는 `POST .../[id]/status` 전용 라우트가 `setUserStatus`에 위임한다(finding E).
...
- **ActorContext는 항상 라우트에서 구성한다.** `requirePermission` 통과 후 `getPermissionSummary(uid)`를 호출하고 그 결과(`{keys, isOwner}`)를 `buildActorCtx(session.user, summary)`에 그대로 넘긴다. service가 `permissionKeys`로 ALLOW override 한도(D13ⓒ)를, `isOwner`로 OWNER-only 경계를 검증하므로 summary를 빼먹으면 가드가 오작동한다.
- **Caution(권위·finding 3) — `isOwner`는 권한 결정과 동일한 단일 DB read에서 온다.** `buildActorCtx`는 `isOwner`를 **`session.user.systemRole`이 아니라 `summary.isOwner`**(=`getPermissionSummary`가 `loadUserContext` 한 번에 산출)에서 가져온다. 이전엔 isOwner를 세션 스냅샷에서, keys를 별도 DB read에서 뽑아 **auth()~권한검사 사이 OWNER 강등 TOCTOU**가 있었다 — 두 값을 같은 read에 묶어 제거한다. `getPermissionSummary`는 must-change/비활성이면 `isOwner=false`(task-07, fail-closed)라 강등·비활성 actor는 anti-escalation에서 비-OWNER로 취급된다. 잔여(같은 요청 핸들러 내 summary read→service mutation 구간)는 target-state 변경에 한해 finding H/1의 락 안 recheck가 추가로 막는다. `session.user.systemRole`은 더 이상 권위 입력이 아니므로 isOwner 판정에 쓰지 마라(여전히 task-07 콜백이 DB fresh로 재구성하지만, ActorContext는 summary를 단일 출처로 쓴다 — SSOT).
...
- **상태 변경은 PATCH가 아니라 전용 라우트(finding E).** `POST /api/admin/users/[id]/status`(body `{status}`)만 `setUserStatus`를 호출해 D14 세션무효화·재활성을 일으킨다. `PATCH /[id]`로 `{status}`를 보내면 `updateUserSchema`에 없는 키라 zod가 strip → 빈 patch가 되므로 step 5에서 **키 0개를 400으로 거부**한다(상태 미변경인데 200 성공을 반환하는 누수 방지). status 전이의 anti-escalation·최소가용성·세션무효화는 service/repository가 강제하므로 status 라우트도 다른 라우트처럼 위임만 하고 가드 로직을 넣지 않는다.
