...
- **ActorContext는 항상 라우트에서 구성한다.** `requirePermission` 통과 후 `getPermissionSummary(uid).keys`로 `permissionKeys`를 채운다. `isOwner`는 세션 `systemRole === "OWNER"`로 판정(세션 신뢰 — auth 콜백이 systemRole을 토큰에 넣음). service가 `permissionKeys`로 ALLOW override 한도(D13ⓒ)를 검증하므로 summary를 빼먹으면 가드가 오작동한다.
