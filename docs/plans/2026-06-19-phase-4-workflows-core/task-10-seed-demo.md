# Task 10 — seed-demo: WorkflowType 3종 + 샘플 task/event/mail

dev 전용 `seed-demo.ts`를 확장해 timeline shell·메일 배지·재시도/해소 UI를 시연·수기검증할 수 있는 데모 데이터를 만든다. **메인 `seed.ts`(prod/cutover)에는 넣지 않는다**(가짜 데이터 권위화 방지 — 기존 분리 원칙).
...
- 기존 `prisma/seed-demo.ts`(WEEKLY_REPORT 타입 + `sample-task-1` + 데모 leave; dev 전용 분리 사유 주석).
- 엔트리포인트 §Shared Contracts **SC-1**(MailDelivery.status 필수·sentAt nullable, WorkflowTaskEvent).
- Spec §10(seed에 WorkflowType 3종 + 샘플 task/event로 shell 시연).
- 이 태스크는 데이터 정의(설정성)이라 TDD 대신 기계적 작성 — 검증은 typecheck/lint + 수동 실행(DB 필요).

## Deps

- Task 01(스키마 — `WorkflowTaskEvent`, `MailDelivery.status/bodyHtml`).

## Step 1 — seed-demo.ts 재작성

`prisma/seed-demo.ts` 전체를 다음으로 교체(기존 `sample-task-1`·데모 leave 보존 + 워크플로 데모 확장):
...
```bash
npm run typecheck   # seed-demo가 새 Prisma 타입(status/bodyHtml/WorkflowTaskEvent)과 정합
npm run lint
```

## Step 3 — 수동 실행 (DB 연결 시, deferred)

```bash
# 메인 seed로 OWNER·권한 먼저 부트스트랩한 뒤
npm run db:seed
# 데모 데이터 적재(dev 전용)
npm run db:seed:demo
```

기대 로그: `[seed-demo] WorkflowType 3종 + task/event/mail + LeaveRequest 데모 seed 완료(dev 전용).`

## Step 4 — commit

```bash
git add prisma/seed-demo.ts
git commit -m "chore(seed): demo WorkflowType x3 + tasks/events/mail (FAILED·SENT·SENDING) for shell"
```

## Acceptance Criteria

```bash
npm run typecheck   # 통과(새 스키마 타입과 정합)
npm run lint        # 통과
npm test            # 전체 통과(seed-demo는 테스트에서 import되지 않음 — 회귀 없음)
```

## Cautions

- **이 데모 데이터를 `prisma/seed.ts`(메인)로 옮기지 말 것.** prod/cutover에 가짜 task/mail이 들어가면 권위 데이터를 오염시킨다(기존 seed-demo 분리 사유).
- **모든 데모 레코드는 고정 id + upsert로 멱등하게.** 재실행 시 이벤트·메일이 중복 누적되면 timeline이 거짓이 된다(`WorkflowTaskEvent`/`MailDelivery`는 cuid 기본이라 반드시 명시 id).
- `MailDelivery`는 `status`가 필수다 — 모든 create에 명시(FAILED/SENT/SENDING). SENT만 `sentAt`을 채우고 FAILED/SENDING은 `sentAt: null`.
- OWNER가 없으면 데모를 건너뛴다(메인 seed 선행 가정) — 조용히 깨지지 않게 경고 후 정상 종료.
