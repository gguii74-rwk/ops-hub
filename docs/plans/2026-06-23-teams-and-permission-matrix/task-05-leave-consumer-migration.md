...
export async function listApprovalQueue(actorId: string) {
  ...
  if (!me.team?.active) throw new ForbiddenError("비활성 팀에서는 승인 대기 목록을 볼 수 없습니다."); // F-R: 폐기 팀은 authz 경계 불가
...
// 1) actor·applicant 행을 id 정렬 순서로 한 번에 잠금(F-P deadlock 회피). 승인은 항상 actor+applicant 둘 다
//    관여하므로 scope와 무관하게 전체 집합을 정렬 순서로 잠근다. actor만 먼저 단독 잠그면(이전 F-O안) 그 뒤
//    정렬-락이 깨져 상호승인(A→B는 A,B / B→A는 B,A 순)이 교차 데드락(F-P). 정렬 1회 획득으로 제거.
...
if (scope === "team") {
  // 3) actor·applicant 현재 팀 비교(락은 1에서 정렬 순서로 이미 보유 — 재배정 차단).
  ...
  // 4) F-R: 폐기된 팀은 authz 경계가 될 수 없다 — 공유 팀이 active일 때만 team-scope 승인 유효(fail-closed).
  const team = await tx.team.findUnique({ where: { id: actorTeam }, select: { active: true } });
  if (!team?.active) throw new ForbiddenError("비활성 팀에서는 team-scope 승인을 할 수 없습니다."); // 롤백
}
...
export async function getRequest(id: string, ctx: LeaveCtx) {
  ...
  if (req.status === "PENDING") {
    ...
    if (scope === "team") {
      ...
      // 무소속(null)·타 팀·비활성 팀(F-R) → 거부(F9). 공유 팀이 active일 때만 team-scope 단건 조회 허용.
...
export async function getLeaveAdminRecipients(applicantTeamId: string | null): Promise<string[]> {
  // F-R: 신청자 팀이 비활성이면 team-scope 승인자는 승인 권한이 없으므로(런타임 active 요구) 통지 대상에서도 제외 → all-scope만.
  ...
  // 팀장(D12④) — 불변식상 lead ∈ 그 팀 active 소속원(F3). 비활성 팀(F-R)·무효 lead는 제외.
...
- **보안 negative(F9·F3·PD3·F-D 필수 — 신규 케이스 추가)**:
  ...
  - **F-R 비활성 팀 authz 경계(필수):** team active=false로 두고 — `listApprovalQueue`→ForbiddenError, in-tx `approveTx`(getEffectiveScope→"team", actor/applicant 동일 팀이나 `team.findUnique→{active:false}` mock)→ForbiddenError(status/메일 롤백), `getRequest` 자기 팀 PENDING 단건→ForbiddenError. team active=true면 기존대로 통과(회귀). `getLeaveAdminRecipients`: 신청자 팀 비활성 → team-scope 승인자·팀장 제외(all-scope만).
  - **F-P 상호승인 데드락 회피(락 순서 불변식):** `approveTx`의 in-tx 잠금이 `{actorId,applicantId}`를 **정렬 순서로 1회** 획득함을 단언(`$queryRaw FOR UPDATE` 호출 인자 순서 검증) — actor-first 단독 잠금이 선행하지 않아야 한다. (실제 데드락 abort는 단위테스트 재현 곤란 → 락 순서 불변식 단언으로 회귀 방지; 통합 환경 상호승인 동시성은 수동 AC.)
...
- **F-R 비활성 팀 경계**: team active=false면 `listApprovalQueue`/in-tx `approveTx`·`rejectRequest`/`getRequest` 모두 team-scope에서 ForbiddenError, recipients에서 team-scope 제외. all-scope 승인자는 영향 없음.
- **F-P 락 순서**: in-tx 잠금이 `{actor,applicant}` 정렬 순서로 1회 획득(상호승인 데드락 회피) — 테스트 GREEN.
...
- **Don't** in-tx 잠금에서 actor를 먼저 단독으로 `FOR UPDATE`한 뒤 정렬 순서로 다시 잠근다. Reason: 정렬 규율이 깨져 상호승인(A↔B) 데드락(F-P). `{actor,applicant}` 정렬 집합을 1회만 잠근다.
- **Don't** team-scope 비교에서 팀 `active`를 생략한다. Reason: 비활성화된 팀이 여전히 승인/조회 authz 경계로 작동(F-R). 모든 team-scope 검사는 공유 팀 active를 fail-closed로 요구.
