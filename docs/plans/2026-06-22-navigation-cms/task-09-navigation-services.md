...
- 기존 출처: `src/modules/admin/users/services/index.ts`(서비스 패턴), `src/kernel/access`(`requirePermission`), `src/kernel/audit`(`writeAudit`).
...
- **audit는 변경 성공 후** 기록(`writeAudit(prisma, ...)`). 실패 시 기록하지 않는다(repo throw가 먼저).
- 권한 거부(requirePermission throw) 시 repo·audit가 호출되면 안 된다(게이트가 최우선).
...
vi.mock("@/kernel/audit", () => ({ writeAudit: writeAuditMock }));
...
describe("navigation services — 게이트·audit", () => {
  it("create: configure 게이트 → createItem → audit", async () => {
    repo.createItem.mockResolvedValue({ id: "n1" });
    const out = await createNavigationItem("admin1", { label: "메뉴", href: "/x", parentId: null, requiredPermissionId: null });
  ...
  it("권한 거부 시 repo·audit 미호출", async () => {
  ...
  it("update: 게이트 → updateItem(CAS) → audit", async () => {
    const at = new Date("2026-06-22T00:00:00Z");
    await updateNavigationItem("admin1", "n1", { label: "새이름" }, at);
    expect(repo.updateItem).toHaveBeenCalledWith("n1", { label: "새이름" }, at);
    expect(writeAuditMock).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: "update", entityId: "n1" }));
  });

  it("delete: 캡처 자식으로 cascadeDelete(parentUpdatedAt=클라 버전) + audit", async () => {
    const at = new Date("2026-06-22T00:00:00Z");
    repo.getNodeForDelete.mockResolvedValue({ children: [{ id: "c1", updatedAt: at }] });
    await deleteNavigationItem("admin1", "p1", at);
    expect(repo.cascadeDelete).toHaveBeenCalledWith({ parentId: "p1", parentUpdatedAt: at, children: [{ id: "c1", updatedAt: at }] });
    expect(writeAuditMock).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: "delete", entityId: "p1" }));
  });

  it("delete: 노드 없으면 Conflict(cascade·audit 미호출)", async () => {
  ...
  it("reparent: 게이트 → reparentItem(id·newParentId·expectedUpdatedAt)", async () => {
    const at = new Date("2026-06-22T00:00:00Z");
    await reparentNavigationItem("admin1", "a", "b", at);
    expect(requirePermissionMock).toHaveBeenCalledWith("admin1", "admin.navigation", "configure");
    expect(repo.reparentItem).toHaveBeenCalledWith({ id: "a", newParentId: "b", expectedUpdatedAt: at });
  });

  it("reorder: 게이트 → reorderSiblings", async () => {
    await reorderNavigationItems("admin1", { parentId: null, orderedIds: ["a", "b"] });
    expect(repo.reorderSiblings).toHaveBeenCalledWith({ parentId: null, orderedIds: ["a", "b"] });
  });
...
export async function updateNavigationItem(
  ...
  await requirePermission(actorId, RESOURCE, "configure");
  await updateItem(id, patch, expectedUpdatedAt);
  await writeAudit(prisma, { actorId, entityType: "NavigationItem", entityId: id, action: "update", metadata: { patch } });
}

export async function reorderNavigationItems(actorId: string, input: ReorderNavInput): Promise<void> {
  await requirePermission(actorId, RESOURCE, "configure");
  await reorderSiblings(input);
  await writeAudit(prisma, {
...
export async function reparentNavigationItem(
  ...
  await requirePermission(actorId, RESOURCE, "configure");
  await reparentItem({ id, newParentId, expectedUpdatedAt });
  await writeAudit(prisma, { actorId, entityType: "NavigationItem", entityId: id, action: "reparent", metadata: { newParentId } });
}

// 삭제(F-6): 직속 자식 캡처 → cascadeDelete(parentUpdatedAt=클라가 본 버전). 노드 없으면 Conflict.
export async function deleteNavigationItem(actorId: string, id: string, expectedUpdatedAt: Date): Promise<void> {
  ...
  await cascadeDelete({ parentId: id, parentUpdatedAt: expectedUpdatedAt, children: captured.children });
  await writeAudit(prisma, {
    actorId, entityType: "NavigationItem", entityId: id, action: "delete",
    metadata: { childCount: captured.children.length },
  });
}
```
