...
describe("cascadeDelete (F-6)", () => {
  it("정상: captured 자식 전부 CAS 삭제 후 부모 삭제(트리락 사용)", async () => {
  ...
  it("reparent-away 자식(CAS count 0): 영향행수 불일치 → 롤백, 부모 삭제 미호출", async () => {
    h.db.navigationItem.deleteMany
  ...
  it("자식 없는 노드(leaf): 부모만 CAS 삭제", async () => {
    h.db.navigationItem.deleteMany.mockResolvedValueOnce({ count: 1 });
    await cascadeDelete({ parentId: "p1", parentUpdatedAt: pAt, children: [] });
...
describe("reparentItem (F-7)", () => {
  ...
  it("정상 승격(newParentId null): 락 + CAS 이동", async () => {
    ...
    await reparentItem({ id: "a", newParentId: null, expectedUpdatedAt: expectedAt });
  ...
  it("정상 이동(top-level 부모, 무자식): CAS 이동", async () => {
    ...
    h.db.navigationItem.updateMany.mockResolvedValue({ count: 1 });
    await reparentItem({ id: "a", newParentId: "b", expectedUpdatedAt: expectedAt });
    expect(h.db.navigationItem.updateMany).toHaveBeenCalledWith(expect.objectContaining({
      data: { parentId: "b", sortOrder: 10 },
    }));
  });

  it("F-7: 대상 부모가 이미 자식이면(depth 위반) ValidationError", async () => {
    h.db.navigationItem.findUnique
      .mockResolvedValueOnce({ updatedAt: expectedAt, parentId: null }) // 이동 노드
      .mockResolvedValueOnce({ parentId: "g1" });                        // 대상 부모가 중메뉴
    await expect(reparentItem({ id: "a", newParentId: "b", expectedUpdatedAt: expectedAt }))
  ...
  it("자기 자신을 부모로: ValidationError(순환 차단)", async () => {
    await expect(reparentItem({ id: "a", newParentId: "a", expectedUpdatedAt: expectedAt }))
      .rejects.toBeInstanceOf(NavigationValidationError);
  });
  ...
  it("stale updatedAt: Conflict", async () => {
    ...
    await expect(reparentItem({ id: "a", newParentId: null, expectedUpdatedAt: expectedAt }))
      .rejects.toBeInstanceOf(NavigationConflictError);
  });
});
```
...
(task-07의 `import type { Prisma } from "@prisma/client";`를 위 값 import로 교체. `Prisma.TransactionClient` 타입 용도는 그대로 동작.)
