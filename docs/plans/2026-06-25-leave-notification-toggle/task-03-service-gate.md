...
```ts
  ...
  it("getSetting 조회 실패(인프라 장애)여도 발송 유지 — fail-open(D4: 알림 누락 > 발송 위험)", async () => {
    ...
    await createLeaveRequest("u1", input);
    const [, mailJob] = repo.createPendingRequest.mock.calls[0];
    expect(mailJob).not.toBeNull(); // 조회 실패 → 발송 유지(D4 의도된 fail-open — 향후 fail-closed로 "수정" 금지)
...
- `npx vitest run tests/modules/leave/mail-wiring.test.ts` — ON(기존 8케이스 무손상) + OFF 3케이스(onRequest/onApprove/onReject) + fail-open(getSetting 실패→발송 유지, D4) + D3 회귀 통과.
