...
- `npx vitest run tests/modules/leave/mail-wiring.test.ts` — ON(기존 8케이스 무손상) + OFF 3케이스(onRequest/onApprove/onReject) + fail-closed(getSetting 예외→미발송, D4 개정) + D3 회귀 통과.
...
- **명시적 `=== true` 비교 + 예외 catch를 써라(D4 개정).** `getSetting` 반환은 `unknown`. `=== true`로 "명시적 true일 때만 발송", 조회 **예외는 catch해 `false`(fail-closed)**. truthy 판정(`if (enabled)`)이나 `!== false`(옛 fail-open)는 쓰지 마라. 미설정/무효 저장값은 `getSetting`이 default `true`를 반환(예외 아님)하므로 ON이 보존된다 — fail-closed는 오직 조회 예외 경로다.
...
- **`createLeaveRequestByAdmin`을 게이트하지 마라**(D3). 중복 게이트는 검토에서 "토글 누락"으로 오인할 수 있으나 의도된 결정이다.
- **읽기 실패 시 leave mutation 자체를 막지 마라(R5 H, ACCEPTED).** 설정 읽기 실패로 연차 신청/승인/반려를 차단하면 설정-DB 블립이 핵심 업무를 마비시킨다 — phase-5 "메일↔업무 성공 분리" 위반. 읽기 실패는 fail-closed 미발송 + alertable error 로그까지만 하고, mutation은 진행한다. durable outbox(SKIPPED 행)는 두지 않는다(D2 no-enum-change + 동일-DB라 near-unreachable 경로 — 과설계). 단 운영에서 이 마커가 실제로 관측되면 durable trail을 재검토할 신호.
