...
- **주의(R4 검토)**: `EXTRA_PERMISSIONS` 추가는 Permission 행만 만든다. **fresh install**은 `bootstrapRolePermissions`가 `pm:"*"`로 grant하지만, **기존 DB**(RolePermission 행 존재)는 bootstrap이 스킵돼 pm이 신규 권한을 **못 받는다** → 기존 배포에서 토글이 403/미노출. 기존 DB grant는 **task-05의 멱등 upgrade-once 헬퍼**가 담당한다(이 task는 Permission 행 생성·catalog까지만).
...
> 이 항목은 **Permission 행만** 생성한다(fresh install은 `bootstrapRolePermissions`의 pm `"*"`가 grant). 기존(비어있지 않은) DB의 pm grant는 task-05의 upgrade-once 헬퍼가 멱등 처리한다 — **이 task만으로는 기존 배포 PM이 토글을 못 쓴다**(R4 검토). OWNER는 systemRole로 항상 허용, 위임 `admin` 역할은 미보유(의도된 경계, D6).
...
- **이 task는 권한을 "등록"만 한다. 기존 DB의 pm "부여"는 task-05.** fresh install은 bootstrap이 pm에 grant하지만, 기존 배포는 bootstrap 스킵 → task-05 upgrade-once가 없으면 PM이 토글을 못 쓴다(R4 검토). 두 task를 함께 배포해야 완결.
