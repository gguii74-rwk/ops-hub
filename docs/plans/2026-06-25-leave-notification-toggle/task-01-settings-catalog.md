...
- `leave.admin` 리소스는 이미 `RESOURCES`에 존재(`leave.admin:view` 사용 중). `configure` 액션만 `EXTRA_PERMISSIONS`에 추가 → Permission 행 생성.
- **주의(R4 적대검증)**: `EXTRA_PERMISSIONS` 추가는 Permission 행만 만든다. **fresh install**은 `bootstrapRolePermissions`가 `pm:"*"`로 grant하지만, **기존 DB**(RolePermission 행 존재)는 bootstrap이 스킵돼 pm이 신규 권한을 **못 받는다** → 기존 배포에서 토글이 403/미노출. 기존 DB grant는 **task-05의 멱등 upgrade-once 헬퍼**가 담당한다(이 task는 Permission 행 생성·catalog까지만).
...
> 이 항목은 **Permission 행만** 생성한다(fresh install은 `bootstrapRolePermissions`의 pm `"*"`가 grant). 기존(비어있지 않은) DB의 pm grant는 task-05의 upgrade-once 헬퍼가 멱등 처리한다 — **이 task만으로는 기존 배포 PM이 토글을 못 쓴다**(R4 적대검증). OWNER는 systemRole로 항상 허용, 위임 `admin` 역할은 미보유(의도된 경계, D6).
...
- **권한은 `leave.admin:configure`(도메인 스코프, D6) — generic `admin.settings:configure`가 아니다.** 기존 도메인 설정(SMTP·weekly)과 동일 패턴. 이렇게 해야 leave 권한 없는 위임 user-admin이 연차 메일을 못 끈다. `EXTRA_PERMISSIONS` 추가를 빠뜨리면 Permission 행이 없어 task-05 upgrade가 fail-closed throw한다.
- **이 task는 권한을 "등록"만 한다. 기존 DB의 pm "부여"는 task-05.** fresh install은 bootstrap이 pm에 grant하지만, 기존 배포는 bootstrap 스킵 → task-05 upgrade-once가 없으면 PM이 토글을 못 쓴다(R4 적대검증). 두 task를 함께 배포해야 완결.
...
- **배포 시 `npm run db:seed` 필요** — `leave.admin:configure` Permission 등록 + task-05 upgrade 실행(task-04의 nav 등록과 함께).
