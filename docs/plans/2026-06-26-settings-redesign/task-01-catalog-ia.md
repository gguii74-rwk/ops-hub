...
```ts
  it("getEntry는 등록 key를 찾고 미등록은 undefined", () => {
    expect(getEntry("integrations.smtp.port")?.kind).toBe("systemSetting");
...
```ts
  it("카탈로그 항목 수 고정 (7 systemSetting, 5 envSecret, 1 relational)", () => {
    const byKind = (k: string) => CATALOG.filter((e) => e.kind === k).length;
    expect(byKind("systemSetting")).toBe(7);
...
```ts
  it("SMTP host systemSetting은 제거됨(env 전용, F4), secure/user도 미추가", () => {
    ...
    // DB 편집 가능한 SMTP 필드는 port·fromAddress뿐
    expect(getEntry("integrations.smtp.port")?.kind).toBe("systemSetting");
...
export const CATALOG: readonly SettingEntry[] = [
  ...
  // NOTE: integrations.smtp.host 는 제거됨(env 전용, F4). host/user/secure/password는 env 신뢰경계 유지.
  {
    kind: "systemSetting",
...
**(d) listSettings "권한 있는 항목만 포함"** (line 143-149): host→port:
```ts
  it("권한 있는 항목만 포함(hasPermission 기준)", async () => {
    ...
    expect(keys).toContain("integrations.smtp.port");
...
> 다른 host-무관 케이스("row 없음 → default" port 587, "envSecret status=coarse" secret.smtp, "relational status=LINK")는 **그대로 둔다**. secret.smtp 행 상태(F12)는 task-05에서 갱신한다.
...
- **Don't `integrations.smtp.host`/`secure`/`user`를 카탈로그에 남기지 마라.** Reason: env 전용(D2·F4). host를 DB 편집 가능하게 두면 전역 env 비밀번호가 임의 host로 유출되는 벡터가 생긴다.
