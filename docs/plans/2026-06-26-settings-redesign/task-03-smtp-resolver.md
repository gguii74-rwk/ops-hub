...
- **Don't port를 DB(`readRaw`/`getSetting`)로 읽지 마라 — port는 env 전용(P3/A2).** Reason: port와 TLS 모드(`secure`)는 SMTP에서 결합돼 있어 같은 출처(env)에서 관리해야 드리프트가 없다. port를 DB 편집하게 두면 env `SMTP_SECURE`와 어긋나 상태는 "정상"이나 미연결인 조합이 생긴다(R3 지적). port는 `process.env.SMTP_PORT`만 본다(DB row는 orphan).
...
- **Note(P3 해소):** `port`·`secure`(TLS 모드) 둘 다 **env 전용**이라 env 한 곳에서 관리되어 어긋날 수 없다(R3에서 지적된 port/TLS 드리프트를 회피 대신 **제거**). 둘 중 어느 것도 DB로 끌어오지 마라 — DB 편집 SMTP 필드는 `fromAddress` 하나뿐. (port를 DB로 되살리면 P3 재발.)
