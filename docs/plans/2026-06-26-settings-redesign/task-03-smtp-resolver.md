...
- **Note(P3 — port/TLS 결합):** `port`는 DB 편집이지만 `secure`(TLS 모드)는 env 전용(D2·F4)이다. 둘은 SMTP에서 결합돼 있어(465↔implicit TLS, 587↔STARTTLS) DB port와 env `SMTP_SECURE`가 어긋나면 상태는 "정상"이라도 실제 연결이 안 될 수 있다. 이는 **F3이 수용한 "배지≠발송보장"의 부분집합**(정적 검증으로 도달성 확인 불가)이며, 보완은 후속 "연결 테스트" 액션(spec §10)이다. secure를 DB로 끌어오지 마라 — F4 보안 결정(민감 연결필드 env 유지)을 되돌리는 것.
