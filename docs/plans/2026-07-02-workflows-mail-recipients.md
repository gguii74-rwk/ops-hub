...
- 게이트(서비스 소유·**서비스가 강제** — billing config 등 workflows 서비스의 단일 권위 패턴): `services/mail-recipients.ts`의
  `canManageMailRecipients(userId): Promise<boolean>` = `hasPermission(admin.settings, configure) && hasPermission(workflows.mail, configure)`(페이지 redirect 판단용) + `requireManageMailRecipients(userId)`(불충족 시 `ForbiddenError`). **모든 서비스 함수가 첫 인자 `userId`를 받아 내부에서 강제** — 라우트 규율에 의존하지 않음(후속 서버 컴포넌트/라우트가 서비스를 직접 재사용해도 fail-closed). 라우트는 401 + `mapError`(ForbiddenError→403)만.
- 라우트(전부 401 → zod/kind 400, 403은 서비스 ForbiddenError→`mapError`): `GET/POST /api/workflows/mail/contacts`, `PATCH/DELETE /api/workflows/mail/contacts/[id]`(PATCH=name·memo만, email 포함 body 400 — D15), `GET /api/workflows/mail/recipients`, `PUT /api/workflows/mail/recipients/[kind]`(kind∉`mailRecipientKinds()` 400, **step 키 집합 = `sendStepsForKind(kind)` 정확 일치 — 누락·초과 모두 400**: 전체 교체 계약에서 부분 body가 다른 단계 세트를 지우지 못하게).
...
- 서비스(`services/mail-recipients.ts` — **전 함수 첫 인자 `userId`, 내부 게이트 강제**): `listMailContacts(userId)`, `addMailContact(userId, {email,name,memo?})`(email trim+소문자, P2002→ConflictError→409), `editMailContact(userId, id, {name,memo?})`(없으면 null→404), `removeMailContact(userId, id)`, `getRecipientSets(userId): RecipientSetView[]`(`{kind, steps, recipients}` — 미저장 step은 빈 필드), `saveRecipientSet(userId, kind, map)`(필드별 `normalizeStoredEmails` 후 전체 교체, type 행 없으면 null→404).
