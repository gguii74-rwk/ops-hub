...
import { approveTx, cancelTx, findOverlap } from "@/modules/leave/repositories";
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (existing.status === "APPROVED") {
      ...
      if (oldYear === newYear) {
        ...
        await tx.leaveAllocation.updateMany({ where: { userId: existing.userId, year: oldYear }, data: { usedDays: { increment: diff } } });
      } else {
        await tx.leaveAllocation.updateMany({ where: { userId: existing.userId, year: oldYear }, data: { usedDays: { decrement: existing.days } } });
        await tx.leaveAllocation.updateMany({ where: { userId: existing.userId, year: newYear }, data: { usedDays: { increment: patch.newDays } } });
      }
    }
    return updated;
  });
...
export async function recalculateUsedDaysTx(userId: string, year: number): Promise<number> {
  return prisma.$transaction(async (tx) => {
    ...
    await tx.leaveAllocation.updateMany({ where: { userId, year }, data: { usedDays: used } });
    return used;
  });
}
```
