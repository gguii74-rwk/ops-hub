...
- **Don't 연도 공휴일을 건별 비원자 write로 적재하지 말 것.** Reason: 일부만 쓰고 실패하면 `count>0`이라 `ensureYearsSynced`가 영구 skip → 누락 공휴일이 평일 처리돼 연차 과다 차감. 연도 전량을 **단일 `$transaction`**으로 → 부분 적재 시 롤백되어 `count===0` 유지·재시도.
- **Don't 네트워크 fetch를 `$transaction` 안에 넣지 말 것.** Reason: 12개월 호출 동안 DB 커넥션·트랜잭션을 점유 → 풀 고갈·tx 타임아웃. `fetchHolidays`는 밖에서 끝내고 write만 트랜잭션.
