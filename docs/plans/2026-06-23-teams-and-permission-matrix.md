...
**in-tx-authz 패턴(F-B/F-D/F-E/F-H/F-O/F-Q) 관측:** "authz를 mutating 트랜잭션 밖에서 함" 계열이 승인·팀장·매트릭스·**override 부여** surface에서 반복 → 가변 멤버십/권한 race. 6 surface 모두 동일 패턴(행 `FOR UPDATE` + tx 내부 재확인/재해석)으로 닫음. **F-P**는 그 락-획득이 도입한 별개 데드락(순서 위반)으로 정렬-1회-획득으로 정정. **any-scope summary(D5) 교훈:** 키-존재를 서버 authz로 쓰는 모든 경로(데이터 가드·override 부여·UI 게이트·승인)를 scope-aware로 — 구현 시 `permissionKeys.has`/`useCan` 전수 재감사(PD3).

R1~R7에서 **F-A~F-R(18건) 닫음(17 FIXED·1 ACCEPTED F-F).** 각 라운드 신규 finding은 다른 부위의 실제 갭(re-flag 아님 — 앞 라운드 finding은 후속 라운드에서 모두 소거 확인). 사용자 승인 검증 연장(R6→R7)이 critical(F-N)·데드락(F-P)·override in-tx(F-Q)·비활성 팀 경계(F-R)를 추가로 잡음 — 연장이 가치 있었음. R7 fix(F-P/F-Q/F-R)는 R8 최종 확인 대기.
