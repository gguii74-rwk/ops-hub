...
- **F-P**(high, R7) — 승인 tx가 actor 행을 **먼저 단독** `FOR UPDATE`한 뒤 `{actor,applicant}` 정렬 순서로 다시 잠금 → 상호승인(A→B, B→A)이 A,B vs B,A 순으로 교차 **데드락**(F-O 수정이 도입한 락 순서 위반). → actor-first 단독 잠금 제거, `{actor,applicant}` 정렬 집합을 **1회만** 획득(task-05 §c-1 + 락 순서 불변식 테스트·상호승인 동시성 수동 AC).
- **F-Q**(high, R7) — override 부여 anti-escalation 가드(F-N)가 여전히 쓰기 트랜잭션 **밖**(service)에서 실행 → 권한 변경 primitive에서 precheck↔write 사이 actor revoke/disable/deny 시 stale actor가 override 부여/제거(in-tx-authz 계열 **5번째 surface**, 최고 민감). **사용자 결정=FIX**(ACCEPTED class 대신): guard에 optional tx + `upsertOverride`/`removeOverride`를 actor `FOR UPDATE` 트랜잭션으로 감싸 in-tx `getEffectiveScope(tx)` 재해석, `createOverride`/`deleteOverride`는 주입 tx에 합류(task-04 §7b + in-tx 원자성 테스트). 승인·매트릭스와 동일 in-tx authz 표준으로 통일.
- **F-R**(high, R7) — 팀 비활성화가 `active=false`+lead 해제만 하고 멤버 `teamId`는 보존 → 폐기된 팀이 여전히 team-scope 승인/조회의 authz 경계(승인 필터가 `team.active` 미확인 — 신규 갭). **사용자 결정=team-scope 체크서 active 요구**(멤버 자동 무소속·비활성화 거부는 기각): `listApprovalQueue`·in-tx `approveTx`/`rejectRequest`·`getRequest` 단건·`getLeaveAdminRecipients`가 공유 팀 `active`를 fail-closed로 요구(task-05 + 비활성 팀 negative 테스트), 결정 노트는 task-03.

- **F-S**(high, R8, **ACCEPTED**) — 승인 in-tx authz의 Read Committed 잔여 창: actor/applicant 행은 잠그나 RolePermission/override 행은 잠금 밖이라, `getEffectiveScope(tx)`↔status CAS 사이(ms)에 권한 폐지가 commit되면 방금 폐지된 승인자가 1회 승인 가능(F-B/F-D/F-O가 닫은 승인 surface의 **4번째 재지목** — in-tx-authz 계열 잔여). **사용자 결정=ACCEPTED.** 근거: 단일 pm2·admin 전용·16명·ms 창; 승인은 가역(cancel/reject)·`auditLog` 기록으로 사후 탐지. 보상 통제=감사 로그. 완전 폐쇄(SERIALIZABLE+retry/전역 advisory lock)는 위협모델 대비 과도(전역 권한-쓰기 직렬화·5+경로 일관 적용 부담). task-05 §c-1 note에 명시.
- **F-T**(high, R8, FIXED) — department drop 배포 계약이 `migrate deploy → … → pm2 restart`로 **old 코드가 살아있는 채 drop** → PD1의 stop→migrate→start 안전 주장과 모순, 비가역 스키마 경계에서 version skew outage. → 런북을 build → stop(+워커) → backup → migrate deploy → db:seed → smoke → start로 교정(task-07 §6 + AC + Caution). `pm2 restart` 단독 금지.

**in-tx-authz 패턴(F-B/F-D/F-E/F-H/F-O/F-Q) 관측:** "authz를 mutating 트랜잭션 밖에서 함" 계열이 승인·팀장·매트릭스·**override 부여** surface에서 반복 → 가변 멤버십/권한 race. 6 surface 모두 동일 패턴(행 `FOR UPDATE` + tx 내부 재확인/재해석)으로 닫음. **F-P**는 그 락-획득이 도입한 별개 데드락(순서 위반)으로 정렬-1회-획득으로 정정. **F-S**(승인 surface의 Read Committed 잔여 ms 창)은 위협모델상 **ACCEPTED로 종결**(완전 폐쇄는 SERIALIZABLE/전역 lock 필요 — 과도; 보상 통제=감사+가역). **any-scope summary(D5) 교훈:** 키-존재를 서버 authz로 쓰는 모든 경로(데이터 가드·override 부여·UI 게이트·승인)를 scope-aware로 — 구현 시 `permissionKeys.has`/`useCan` 전수 재감사(PD3).

R1~R8에서 **F-A~F-T(20건) 닫음(18 FIXED·2 ACCEPTED[F-F·F-S]).** 각 라운드 신규 finding은 다른 부위의 실제 갭(re-flag 아님 — 앞 라운드 finding은 후속 라운드에서 모두 소거 확인). 사용자 승인 검증 연장(R6→R8)이 critical(F-N)·데드락(F-P)·override in-tx(F-Q)·비활성 팀 경계(F-R)·배포 순서(F-T)를 추가로 잡음 — 연장이 가치 있었음. R8은 in-tx-authz 승인 잔여 창(F-S, 사용자 ACCEPTED)·배포 순서 안전 갭(F-T, FIXED)으로 종결. **plan phase 종료 — 미판정 blocking 0.** impl은 새 세션 `superpowers:subagent-driven-development` task-01부터(빌드 순서 1→2→3·4·5→6→7).
