...
// 다운로드 가능 상태(서버 권위) = 산출물이 생성된 이후·미취소 상태. PENDING(미생성)·CANCELLED(취소)는 거부.
// UI 상태게이트(SC-10)는 클라이언트 전용이라 직접 API 호출로 우회되므로, 같은 불변식을 서버에서 강제한다.
const DOWNLOADABLE_STATUSES = new Set<WorkflowStatus>([
...
export async function getFileForDownload(ctx: DownloadCtx, taskId: string, fileId: string): Promise<FileDownload | null> {
  ...
  if (!DOWNLOADABLE_STATUSES.has(f.status)) return null; // 비다운로드 상태(PENDING/CANCELLED 등) → 404(서버 강제)
  let abs: string;
  try {
...
export async function getDirectoryZip(ctx: DownloadCtx, taskId: string): Promise<ZipDownload | null> {
  ...
  if (!DOWNLOADABLE_STATUSES.has(t.status)) return null; // 비다운로드 상태(CANCELLED 등) → 404(서버 강제)
  let absDir: string;
  try {
