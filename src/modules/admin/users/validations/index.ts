...
const systemRole = z.enum(["OWNER", "ADMIN", "MEMBER"]); // MANAGER 폐지(미사용 coarse 등급) — 신규 부여 차단. DB enum·TS 타입·기존 데이터는 유지.
...
// NF2: name은 선택(기존 호출 호환). 승인이 프로필 권위 — admin 입력값이 사용자 self-input을 덮어쓴다.
