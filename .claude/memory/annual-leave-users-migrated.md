...
- 상세·스크립트: `docs/migration/2026-06-22-annual-leave-users.md`, `scripts/migrate-al-users*.{ts,py}` (**main 머지됨 2026-06-25, 머지커밋 af1338f** — 브랜치 `chore/migrate-al-users`는 삭제됨; create-only·opshub 가드). **연차 데이터(allocation/request)는 미이전**(이번 범위 아님 — Phase 6 운영 cutover에서 스크립트 재사용).
