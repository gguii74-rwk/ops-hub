...
// F-GG/F-FF: withAvailabilityLock(advisory lock $executeRaw) + actor·target FOR UPDATE($queryRaw) + 가드 team 읽기(user.findUnique) tx 스파이.
...
// F-GG: upsertOverride/removeOverride는 withAvailabilityLock(prisma.$transaction + advisory lock)으로 감쌈 — DB 없이
//   동작하도록 passthrough mock. tx에 $executeRaw(advisory lock)·$queryRaw(actor/target FOR UPDATE)·user.findUnique(가드) 제공.
vi.mock("@/lib/prisma", () => ({
  prisma: {
    $transaction: vi.fn(async (fn: (tx: unknown) => unknown) =>
      fn({ $queryRaw: txSpies.queryRaw, $executeRaw: txSpies.executeRaw, user: { findUnique: txSpies.userFindUnique } }),
    ),
  },
}));
...
import {
  approveUser, createUserByAdmin, assignRoles, upsertOverride, removeOverride, resetPassword, updateUser,
...
// F-GG/F-FF — override mutation은 availability advisory lock + actor·target 행 잠금 안에서 수행
describe("F-GG/F-FF override mutation 동시성 가드", () => {
  it("upsertOverride: withAvailabilityLock(advisory) + actor·target FOR UPDATE 안에서 createOverride", async () => {
    await upsertOverride(delegate, "target1", {
      resource: "leave.request", action: "view", effect: "ALLOW", scope: "all", reason: null, startsAt: null, endsAt: null,
    });
    expect(txSpies.executeRaw).toHaveBeenCalled();     // F-GG: availability advisory lock 획득
    expect(txSpies.queryRaw).toHaveBeenCalledTimes(2); // F-FF: actor + target 두 행 잠금(team-move race 직렬화)
    expect(h.repo.createOverride).toHaveBeenCalled();
  });
  it("removeOverride: 동일하게 advisory lock + actor·target 잠금 안에서 deleteOverride", async () => {
    h.repo.getUserDetail.mockResolvedValue(userDetailFixture({
      overrides: [{ id: "ov1", resource: "leave.request", action: "view", effect: "ALLOW", scope: "all" }],
    }) as never);
    await removeOverride(delegate, "target1", "ov1");
